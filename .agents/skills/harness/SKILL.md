...
1. **Read state from disk, every cycle** — your feature's `STATE.md` and `feature.json`, plus the
   ...
   First cycle ever: instantiate `STATE.md` from `.agents/skills/harness/templates/STATE.md`
   and `feature.json` from `.agents/skills/harness/templates/feature.json` — the second is a
...
3. **Delegate to a lead, never a member.** Every dispatch is a plain subagent — **never pass a
   ...
   **In the build phase, dispatch the named `build` team — never compose a step list at dispatch.**
   Resolve it `.harness/teams/build.yaml` first, then `.agents/skills/harness/teams/build.yaml`
   ...
   **After the build team returns, sequence the qa segment.** It is a **validator-squad** segment
   ...
   procedure is `.agents/skills/harness-simplify/SKILL.md`, read at the point of use and
...
`.agents/skills/harness/references/missions.md` before acting — the full procedure lives there
(DEC-158).
...
1. Dispatch **each lead that ran this feature** once: "distill — **read
   `.agents/skills/harness-distill/SKILL.md` first (NOT preloaded, DEC-158) and tell each member to
...
3. Write it to `.harness/harness/features/<FEAT>/notes/ship-review-<runid>.md` — plain English, bounded length,
   ...
   `python3 .agents/skills/harness/bin/render-brief.py <that path>` writes the `.html` sibling. The
