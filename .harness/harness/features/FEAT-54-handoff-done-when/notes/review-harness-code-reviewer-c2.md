...
| F-03 PreToolUse Edit refusal before mutation | **closed** | Handoff Edit joins candidate reconstruction before the tool runs (`check-domain.py:1819-1872`); invalid UTF-8 returns the unreadable sentinel and exit 2 (`:1841-1844,1865-1868`). Gate cases assert invalid candidate exit 2, byte identity after refusal, and non-UTF-8 byte identity (`test-check-domain.py:4114-4166`). |
...
- **SC-07 PASS inspection.** At the pin, the write gate imports/calls only the shared `handoff_done_when.problems(..., resolve=True)` at `check-domain.py:1562-1563`; the state gate imports at `check-state.py:54` and calls the same function with `resolve=False` at `:1251`. Searches of both full pinned gate sources found no second Done-when block parser or pointer-target resolver.
- **SC-08 PASS inspection.** Current-contract presence is explicit in `SKILL.md:311-316`, `templates/HANDOFF.md:4,37-40`, `check-domain.py:1547-1558`, `check-state.py:1069-1070,1212-1251`, and `DECISIONS.md:3701-3727,6698-6701`. The only four-heading gate matches are the authorized FEAT-31 historical measurement/incident at `check-state.py:1199,1218`; `DECISIONS.md:3765` likewise records the historical first live four-section handoff, not the current contract.
