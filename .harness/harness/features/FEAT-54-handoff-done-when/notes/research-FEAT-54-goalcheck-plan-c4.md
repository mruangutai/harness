...
| T-07 | **yes, at HEAD.** The `grep -qi 'done when' check-state.py` conjunct finds nothing today, so the block is red independently of the suite; the real-corpus conjunct then requires no `Done when` line | measured |
