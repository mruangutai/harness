...
- `.claude/skills/harness/bin/check-domain.py:1561-1566` imports that module and calls `problems(..., resolve=True)` rather than restating it.
- `.claude/skills/harness/bin/check-state.py:53-56,1243-1251` imports the same module and calls `problems(..., resolve=False)` rather than restating it.
