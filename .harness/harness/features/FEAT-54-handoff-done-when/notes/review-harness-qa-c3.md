...
- **SC-07 PASS:** `check-domain.py:1561-1566` imports/calls `handoff_done_when.problems(..., resolve=True)` and fails closed; `check-state.sh:53-56,1243-1251` imports/calls the same implementation with `resolve=False`. Neither gate contains another Done-when body parser or pointer resolver.
