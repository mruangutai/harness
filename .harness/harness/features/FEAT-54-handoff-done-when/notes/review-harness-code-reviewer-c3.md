...
- **One parser/resolver:** `check-domain.py:1561-1566` calls `handoff_done_when.problems(..., resolve=True)` and `check-state.sh:53-56,1243-1251` calls the same interface with `resolve=False`. Searches found no second Done-when body parser or pointer resolver in either gate. This satisfies SC-07/DEC-179.
