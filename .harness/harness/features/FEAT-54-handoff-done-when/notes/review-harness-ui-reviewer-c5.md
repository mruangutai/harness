...
5. `.claude/skills/harness/bin/check-domain.py` — author-facing write gate.
...
8. `.claude/skills/harness/bin/check-state.py` — operator-facing state gate.
...
DIGEST:
  ...
  contract_violations:
    - { path: ".claude/skills/harness/bin/check-state.py", actual: "literal root command exit 1; one INV-29 BUG-1157 worktree violation; zero Done when lines", specified: "BRIEF SC-04 and c5 acceptance require a clean literal repository-root state check" }
  a11y: []
  open_questions:
