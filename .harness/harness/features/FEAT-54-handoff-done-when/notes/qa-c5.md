...
- **SC-07 PASS by inspection:** `check-domain.py:1546-1566` imports the single module and calls `problems(..., resolve=True)`; `check-state.sh:53-56,1243-1251` imports it and calls `problems(..., resolve=False)`. Neither gate carries a second Done-when body parser or target resolver.
