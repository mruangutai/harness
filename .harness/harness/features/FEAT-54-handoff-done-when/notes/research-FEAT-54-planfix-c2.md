...
| M-2 | plan.yaml T-06 `intent` | new case (h): a non-baselined 60-line note with a 25-line `## Trust` and a resolving block is reported by NO check-state.py line; declared the state-check half of SC-14 (T-03(h) is the write-gate half), expected GREEN before and after, red the moment INV-17 gains a per-section cap. Expected-state paragraph updated to list (b)(d)(g)(h) as green |
