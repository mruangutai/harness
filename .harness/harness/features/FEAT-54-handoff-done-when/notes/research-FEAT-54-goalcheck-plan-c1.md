...
`test_kinds`: `unit` cmd `run-unit-tests.py --kind unit` (non-null), `integration` cmd
`run-unit-tests.py --kind integration` (non-null); `functional`, `component`, `ui`, `eval`, `typecheck`
are `cmd: null`.
...
(run-unit-tests.py:31); `test-handoff-done-when.py` goes to `UNIT_SCRIPTS` only (D-06, T-01) and is
