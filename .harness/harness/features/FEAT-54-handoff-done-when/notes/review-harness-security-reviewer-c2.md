...
- `.claude/skills/harness/bin/check-domain.py` — **in scope**: untrusted hook JSON, PreToolUse Edit reconstruction, fail-closed exception and exit-2 boundary.
- `.claude/skills/harness/bin/check-state.py` — **in scope**: repository corpus/config input, baseline trust and grammar-only persisted check; exact root invocation executed.
