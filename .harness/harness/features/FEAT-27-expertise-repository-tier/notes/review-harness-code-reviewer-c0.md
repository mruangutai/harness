...
## F-1 (med, NEW) — `check-expertise.py` validates repository-tier segment names that
`inject-expertise.py` will silently never inject

**Premise checked at base (`b4659cd`):** `check-expertise.py` had no tier concept at all
(`LINE_BUDGET = 150` uniform, confirmed by `git show b4659cd:.claude/skills/harness/bin/check-expertise.py`),
...
**The gap.** `check-expertise.py`'s `REPO_TIER_RE` (`check-expertise.py:56`) is
...
`check-expertise.py` to validate segment well-formedness (REQ-04's text is scoped to budget and
...
have `check-expertise.py` print an advisory when a repository-tier path's segment would be
...
- **No-touch constraints honored**: `git diff --stat` confirms `fleet.yaml`, `harness.json`,
  `gh_board.py`, `load_board`, `factory_claim.py`, `check-state.py`, `check-domain.py`,
...
gap in `check-state.py`, under the DEC-174 carve-out) — my review is orthogonal to it and does not
change its status.
