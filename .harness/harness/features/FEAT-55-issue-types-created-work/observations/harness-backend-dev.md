...
- 2026-09-05: on tests/integration/test-gh-sync.py, `edit` reported a successful apply (new snapshot tag) that never reached the real filesystem — a follow-up `read`/`bash sed`/`md5sum` on the same absolute worktree path kept showing the pre-edit content and the old tag, while a retry of the identical edit against the new tag was rejected as "byte-identical" (i.e. the edit tools own internal view believed the new content was already live). Filed as a tool bug via xd://report_issue (rejected by check-domain, not my path) and worked around by patching the file in-place with a python3 heredoc in bash, then confirmed with git status --porcelain that only the intended file changed. G-18 (worktree absolute-path discipline) alone did not catch this; the extra step that did was checking bash/git output, not the tool own read-back, after every edit on this file.
- 2026-09-05: FEAT-55 fix cycle 13 — code-grade.py's --base/--head mode requires an actual commit revision (rev-parse --verify <ref>^{commit}); a bare tree object from git write-tree fails with "invalid Git commit revision". To grade uncommitted worktree edits without moving HEAD or committing: git add the target files, git write-tree for the tree sha, git commit-tree <tree> -p HEAD -m ... for a detached commit object (never checked out, so HEAD stays put), then git reset immediately to restore the unstaged working tree. Pass the detached commit sha as --head.
- 2026-09-05: FEAT-55 — code-grade.py --base --head only reports functions in the DIFF's gated/changed set (by body-hash comparison against base); a function whose edited body happens to hash-match an earlier ancestor state (here: write_factory.transform after extracting the added typed sort back into a helper) drops out of that report entirely, even though it changed in this commit. Use code-grade.py <path> (whole-file mode) to get every function's grade/ABC regardless of diff-membership when a specific function must be reported on.
