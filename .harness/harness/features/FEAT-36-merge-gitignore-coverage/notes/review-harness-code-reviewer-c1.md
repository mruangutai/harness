...
- Executable/config changes: `.agents/skills/harness/bin/test-merge-gitignore.py`, `.agents/skills/harness/bin/run-unit-tests.py`, `.agents/skills/harness/bin/test-bash-write-guard.py`, `.harness/harness.json`. The other 39 changed paths are feature-local approved authority, state, notes, and run records. The production utility and canonical snippet were inspected but are unchanged.
