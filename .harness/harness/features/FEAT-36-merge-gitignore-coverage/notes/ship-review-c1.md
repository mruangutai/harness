...
**Ready for an independent main-targeting PR.** All six success criteria passed in the OpenAI Harness cycle, and the extracted pre-port implementation passed the complete repository suite at `638cb20289ed2aa96448cbaf6a0e7f21249b890a`. The operator-requested B-1 diagnostic-set weakness remains closed with discriminating mutant evidence.

The earlier stacked PR #833 is historical only. This extraction keeps FEAT-36 separate from the OMP port and leaves merge to the new main-targeting PR.
...
- Standalone implementation pin: `638cb20289ed2aa96448cbaf6a0e7f21249b890a`.
- Original OpenAI panel: PASS from code, QA, security, and UI with no in-scope must-fix (`runs/review-c3-validator/digest.md`).
- Standalone pre-port matrix: 21 unit + 23 integration registrations passed, including the changed behavioral program 7/7 directly and through registration; no `MISCONFIGURED` or kind drift.
