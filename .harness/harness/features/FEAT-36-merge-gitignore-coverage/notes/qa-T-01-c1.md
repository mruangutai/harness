...
`run-unit-tests.py:17-18` registers 23 unit and 23 integration scripts; `--kind all` selects their union (`run-unit-tests.py:35-45`). All 46/46 registrations executed successfully in the same aggregate command, including integration registration `test-merge-gitignore.py` (`run-unit-tests.py:18`). No runner `MISCONFIGURED`, `KIND-DRIFT`, or `FAIL test-` diagnostic occurred. The runner's own kind-check regression test also measured zero live `KIND-DRIFT` lines.
