...
| SC-08 | met | unit | unedited: `plan.yaml:88-90` sha256 pins, re-run twice (`feature.yaml gate_status.task_verify`); passing: all three are in `UNIT_SCRIPTS` (`run-unit-tests.py:17`), and the unit kind ran 10/10 scripts (`runs/t01-qa-validator/digest.md` `kind_results`) |
