...
- 2026-08-26 (T-05 cycle 1): a lazy import moved inside one gate function does not cover every
  caller of that function -- require_or_die() has multiple callers (check-state.py at minimum) and
  the fix that satisfied bash-write-guard.py/check-domain.py (neither of which calls
  require_or_die()) left check-state.py broken. When a shared function's caller list is not
