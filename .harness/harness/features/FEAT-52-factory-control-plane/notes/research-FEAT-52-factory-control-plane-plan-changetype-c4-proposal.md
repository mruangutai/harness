...
lanes:
  ...
  rows:
    - surface: .claude/skills/harness/bin/inject-expertise.py
...
decisions:
  - id: D-01
    choice: "The Harness control-plane root reaches an agent as one line of the preamble that inject-expertise.py injects through hookSpecificOutput.additionalContext, and it is the only value this feature injects."
  ...
  - id: D-03
    ...
    dec: DEC-183
  - id: D-04
    choice: "The spawn-time assertion lives inside inject-expertise.py but signals through the injected text and stderr, never through the exit code, which stays 0 on every branch."
...
tasks:
  ...
  - id: T-03
    ...
    change_type: logic
    execution_mode: main-session-direct
    execution_reason: DEC-174 carve-out, inject-expertise.py is a registered SubagentStart hook and its test file is enforcement layer
    depends_on: [T-02]
    status: ready
    files:
    ...
    intent: |
      inject-expertise.py resolves the harness root today (variable `root`, from
  ...
  - id: T-07
    ...
    intent: |
      ...
      .gitignore, bin/merge-settings.py, bin/merge-gitignore.py - are template FILENAMES in the
    ...
    intent: |
      ...
      THE REASONING, in four or five sentences and no more:
        ...
        - why two anchors: measured at sha e8e1b78be3379d4a669aa7e28aef8f76eb942471, settings.json
          registers the MAIN checkout's copy of inject-expertise.py and
      ...
      Record the REJECTED alternatives and why, because a future scan will re-suggest both:
        - injecting a SECOND resolved value from inject-expertise.py. The hook cannot identify the
  ...
  - id: T-14
    ...
    change_type: logic
    execution_mode: main-session-direct
    execution_reason: DEC-174 carve-out, inject-expertise.py is a registered SubagentStart hook and test-inject-expertise.py is its test file
    depends_on: [T-03]
    status: ready
    files:
      - .claude/skills/harness/bin/test-inject-expertise.py
    ...
    intent: |
      ...
      inject-expertise.py contains no non-zero exit on any branch. This is the second clause of
      ...
      2. THE FILE, and the count. Scan the SHIPPED script, resolved from the repository the test
         ...
         Path(__file__).resolve().parent / "inject-expertise.py". Never a copy, never a fixture,
      ...
      4. WHAT THIS CASE MUST NOT DO. It must not execute the hook: this is a text scan of the
         ...
         did not fire. It must not edit inject-expertise.py. And it must not register the test file
         anywhere new - test-inject-expertise.py is already registered in run-unit-tests.py and
