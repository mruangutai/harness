...
2. `.claude/skills/harness/bin/check-state.py`
   ...
   ```
   (check-state.py also emits many `note` lines, unrelated to VIOLATION count, left untouched.)
...
- check-state.py VIOLATION count dropped 5→4 as expected; FEAT-10 no longer named; remaining 4
