...
- 2026-09-11: check-state.py INV-32 emits NO line naming the goalcheck reader for any feature once recorded; the absence grep across the whole 1354-line output is the cleanest proof the gate closed.
...
- 2026-09-11: BUG-285 foldq7 c2 — a decision citing a notes/ file as its able-to-fail evidence can land and be signed while that file does not exist; nothing in check-state.py or check-plan-routes.py resolves a notes path inside a decision's prose, so the dangling citation is invisible to every gate.
