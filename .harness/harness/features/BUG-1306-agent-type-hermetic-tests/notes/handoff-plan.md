# Handoff — BUG-1306, plan → build — written at c369fb1, seq-1
...
- T-01's `verify:` reports RED on the unfixed tree — ran the block extracted from plan.yaml, exit 1, no `VERIFY-OK` — verified-at c369fb1
- The defect reproduces as BRIEF states: governed env 14 `FAIL` lines / exit 1, clean env 0 / exit 0 — orchestrator ran both invocations directly — verified-at c369fb1
- `plan-merge.py:1188` `cmd_sign_approval` is the ONLY production read of `HARNESS_AGENT_TYPE` from env — grep over both `bin/` trees, corroborated by the `scope` reader at source — verified-at c369fb1
- A module-import `os.environ` pop cannot reach a sibling test file — `run_pool.py:63` runs each file as its own `subprocess.run` — verified-at c369fb1
- Panel F1 (high) is CLOSED in the plan, not waived — plan.yaml `panel.findings` PF-8d2608761fd582d9e04a7fe844b2e0da, disposition `resolved` — verified-at c369fb1
...
- Do not widen to a shared `tests/integration/` hermetic helper or any tree-wide env lint — plan.yaml D-01, D-04 — verified-at c369fb1
- Do not edit `plan-merge.py`; its env read is deliberate #1103 defence — plan.yaml D-03 — verified-at c369fb1
- Do not add a new case asserting hermeticity — plan.yaml D-05 — verified-at c369fb1
- Do not touch `tests/integration/test-plan-merge.py:1097-1140`; both `case_1103_*` bodies stay byte-identical — plan.yaml T-01 `intent:` — verified-at c369fb1
