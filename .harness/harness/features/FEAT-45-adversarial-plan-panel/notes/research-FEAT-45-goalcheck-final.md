...
(`check-state.py:213-217` now emits distinct `disposition resolved.` / `disposition overruled.`
...
`cmp`-verified `run-unit-tests.py` and `check-state.py` are byte-identical through both paths, so the
...
| **SC-02a** `scope` rendered output | met | `check-domain.py --resolve .harness/harness/features/FEAT-45-adversarial-plan-panel/notes/review-harness-code-reviewer-planpanel-c0.md` | rc=0, prints `harness-code-reviewer` (+ `harness-orchestrator`) — the step's own persona |
| **SC-02b** playbook goal-check note | met | `check-domain.py --resolve .harness/harness/features/FEAT-45-adversarial-plan-panel/notes/research-FEAT-45-adversarial-plan-panel-goalcheck-plan-c0.md` | rc=0, prints `harness-pm` |
...
| **SC-07** | met | my own fixture: `panel` key absent entirely | rc=1, `VIOLATION INV-32: FEAT-INV32 plan is approved with no complete panel result recorded.` (`check-state.py:181-182`) |
...
| **SC-13** stale overrule refused | met | my own fixture: ruling on `PF-cafebabe`, absent from the current findings | `VIOLATION INV-32: … STALE OVERRIDE PF-cafebabe: a reworded finding gets a NEW content-hash id, so the old ruling stopped applying and the operator is asked again.` (`check-state.py:201-204`) |
...
2. **The mutant must live in the bin directory.** My first attempt placed the D-13 marker mutant in
   `/tmp` and it tracebacked — `check-state.py` resolves its root from `$_SELF_BIN` via
