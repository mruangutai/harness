...
`.harness/harness/features/FEAT-45-adversarial-plan-panel/feature.json`, `BRIEF.md`, `plan.yaml`
and `STATE.md` exist only in the worktree checkout
(`.claude/worktrees/harness/FEAT-45-adversarial-plan-panel/`), not at this path in the main
checkout — only `notes/` and `runs/` are present here, apparently shared/synced across the parallel
review run. All source citations above were read from the worktree, at the pinned SHA (verified
byte-identical to `d0ebbe6` except `feature.json`, per the diff check). This note itself was
written here, at the main-checkout path, after an initial write to the worktree-prefixed path was
rejected by the digest validator because it could not resolve `feature.json` for `code_grade`
binding from that location.
