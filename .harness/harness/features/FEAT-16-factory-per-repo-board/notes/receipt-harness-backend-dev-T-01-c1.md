...
## BLUF
`load_fleet` now accepts a per-repo `board:` block in `repos[]` entries, keeps the top-level
`board:` block OPTIONAL (unchanged validation when present), and rejects only the one new case:
a `repos[]` entry with no board and no fleet-level fallback. Two new resolvers, `board_for` and
`board_station`, resolve through `repo_entry` so an unlisted repo still raises the existing
"repository not in fleet" error. The old two-arg `station(fleet, key)` is untouched (T-08 deletes
it). `--show`'s payload always carries `"repos"`, and carries `"board"` iff the loaded file still
declares a top-level board.

## Payload contract (stated, not just coded)
- `"repos"` always present.
- `"board"` present **iff** the source fleet file declares a top-level board.
- Each `repos[]` entry carries its own `board` mapping verbatim when it declares one.
...
`_validate_board(board, where, path)` — `where=""` for the fleet-level block, `where=f"repos[{name}]"`
for a per-repo block. Internally `key_base = f"{where}.board" if where else "board"`, so the
fleet-level messages keep saying `board.owner`/`board.number`/`board.station_field`/`board.stations`
(and `board` for the not-a-mapping case), and per-repo messages say
`repos[owner/name].board.owner` etc. — matching the intent's stated message shapes AND its literal
call-site text `_validate_board(entry["board"], f"repos[{name}]", path)`.

## tests_added — verified, not recalled
Baseline: ran the pre-task `test-factory-config.py` (via `git show HEAD:...`) with `PYTHONPATH`
pointed at the pre-task `factory_config.py` (also via `git show HEAD:...`) — `58/58 checks passed.`
Post-task: `73/73 checks passed.` **15 tests added** (73 − 58) is the number that matters, because
it is what the suite actually executed. It also matches a manual count of the new named cases:
(25)×2, (26)×2, (27)×2, (28a–d)×4, (29)×2, (30)×1, (31)×2 = 15. (A textual `grep -c` for `check(`
over the diff returns 17 — 3 of those static call sites sit inside the `REPO_BOARD_BAD_CASES` loop
body, where only one branch executes per iteration, so the static line count is not the same
question as the executed-check count; the RAN-count diff is the correct measurement.)

## TDD discipline note (disclosed per P-13)
Production code for this task was written before the new tests were added — out of order. To
reconstruct honest RED evidence rather than skip it: hashed the edited `factory_config.py`
(`sha256: 1f08097969f30adee86da5e1b83010c6e16d74300afaf4f01ac2e802fd2a9dbb`), swapped in
`git show HEAD:` (the pre-task version), and ran the (already-written) `test-factory-config.py`
against it. The run **aborted at the first new case** — case (25), `load_fleet` on a
board-less-at-top-level, per-repo-only fleet raised `FleetError: fleet key invalid: board — set
board: {...} as a mapping in <path>` before it reached a `check()` call, so cases (26)–(31) never
executed against the old code. That single hard failure is the RED evidence: the pre-task loader
cannot load the new fleet shape at all. Restored the edited file, re-verified the hash matched
(`1f08097969f30adee86da5e1b83010c6e16d74300afaf4f01ac2e802fd2a9dbb`, unchanged), and confirmed
`git status --porcelain` showed only the intended two-file diff. Then ran GREEN: 73/73 checks pass
in `test-factory-config.py` standalone, and the full unit suite below.
...
fault-injection fixture inside `test-factory-decompose.py` (a file this task does not touch),
not a failure — that script reports `PASS test-factory-decompose.py` immediately after, and
no `FAIL <script>` line appears anywhere in the output above.

## Notes for T-02 / T-03 / T-04 (downstream)
- `factory_decompose.py:423`'s call to the old two-arg `station(fleet, "ready")` still works — not
  touched by this task, intentionally (per the plan's mechanical reason: removing it here would
  turn this task's own verify red).
- `board_for(fleet, repo_name)["stations"]["review"]` — direct-index idiom — is safe for
  T-04 (factory_land) for the same reason `board_station` is safe: `_validate_board` still enforces
  the stations key set to exactly `ready`/`building`/`review` on every board, fleet-level or
  per-repo.
