# Receipt — harness-backend-dev — T-09 (run c1)

## Task

T-09 — Assert the declared repository-to-board pairing in test-no-distribution.py.

## Verify command cross-check
...
Full unabridged stdout (757 lines; the added case5 block and its three checks are the block
starting `PASS board_lives_per_repo_not_fleet_level` immediately before `ALL PASS` /
...
`main()` in `.claude/skills/harness/bin/test-no-distribution.py` calls `case5()` alongside the
existing calls:
...
```
$ grep -n "case5()" .claude/skills/harness/bin/test-no-distribution.py
258:def case5():
307:    case5()
```

(line 258 is the `def`, line 307 is the call inside `main()`.)

## No network call

`case5()` reads `.harness/factory/fleet.yaml` off local disk with a plain `open()` +
`yaml.safe_load()`, exactly like the existing `case3()` it is modelled on. Established by:
...
Since this task is entirely new test code (no separate production module to RED against),
I proved each of the three new `check()` calls is load-bearing by monkeypatching
`yaml.safe_load` to return three different malformed fleet shapes and confirming each targeted
mutant reddens exactly the check it is meant to catch, while a fourth run against the real,
...
`yaml.safe_load`'s return value was monkeypatched in a throwaway script, and the real file was
read unmodified for the control run and for the actual verify command above.
