...
## BLUF
`factory_decompose.py` now resolves the board it publishes to per repository:
`factory_config.board_for(fleet, args.repo)` replaces the hoisted `fleet["board"]` reads at step
3b (owner/board_number/station_field/stations for the pre-flight station validation), and
`factory_config.board_station(fleet, args.repo, "ready")` replaces the two-argument
`factory_config.station(fleet, "ready")` at step 7. Nothing else changed: the hoist stays before
`ensure_labels`, the ledger/dispositions/parent-handling/record-after-`project_field_set`
ordering is untouched, and the two-argument `station()` function itself is still defined
(T-08 deletes it, not this task).

## Files touched
- `.claude/skills/harness/bin/factory_decompose.py`
- `.claude/skills/harness/bin/test-factory-decompose.py`

## Comment grep criterion
`grep -n "fleet\[.board.\]\|fleet.board\|board.:" factory_decompose.py` returns nothing — no line
in the file, comment included, mentions a fleet-level board any more. The comment above the hoist
now names the per-repo resolution (`factory_config.board_for`) instead of `fleet["board"]`.

## Test changes (T-03's own scope)
- `good_fleet_dict` migrated: the top-level `board` block moved into the single `repos[0]`
  entry's own `board` mapping (no fleet-level board left in this fixture).
- The D4-4 "Redy" typo fixture moved from `bad_fleet["board"]["stations"]["ready"]` to
  `bad_fleet["repos"][0]["board"]["stations"]["ready"]` — case name and assertions unchanged.
- Added `two_repo_fleet_dict`: two `repos[]` entries, no fleet-level board, on two different
  board numbers/owners/station fields/option names (A: acme/3/Status/Ready·Building·Review;
  B: other-org/7/Stage/Other-Ready·Other-Building·Other-Review).
- Added one new case block (`(T-03) ...`, 7 checks) publishing `--repo` A against the two-repo
  fleet, asserting on the recorded `project_item_add`/`project_field_set`/`project_field_options`
  call arguments: every call is addressed to A's board (owner `acme`, number `3`), none is
  addressed to B's (`other-org`, `7`), and the station set uses A's own ready option
  (`Ready`), never B's (`Other-Ready`).

## TDD discipline note (disclosed per P-13)
Production code for this task was edited before the test changes were written — out of order.
To reconstruct honest RED evidence: hashed the edited `factory_decompose.py`
(`sha256: eebe8c96765b7fde5af203d0e68e7dc8e9579ef1cc1b9e23ba2ebb2df3b28208`), swapped in
`git show HEAD:` (the pre-T-03 version — T-01/T-02 never touched this file, so HEAD's copy is
exactly the pre-task code), and ran the already-written `test-factory-decompose.py` against it.
It aborted with `KeyError: 'board'` on case (2) (`fleet["board"]` no longer exists once the
fixture is per-repo-only) and every subsequent case through the file's end either failed or never
ran (a `StopIteration` crashed the script outright partway through case (5), confirming the
migrated fixtures are unloadable by the old code — real RED, not vacuous). Restored the edited
file and re-verified the hash matched (`eebe8c9...`, unchanged), and confirmed
`git status --porcelain` showed only the intended two-file diff. Then ran GREEN: full suite below,
`test-factory-decompose.py` at 181/181.
...
present in the file before T-03 too — not a failure; the script reports
