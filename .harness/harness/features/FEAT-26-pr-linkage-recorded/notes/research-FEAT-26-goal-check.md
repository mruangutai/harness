...
| SC-09 | **MET** | `check-state.py` on this tree emits exactly **one** INV-28 line, and it names `FEAT-24-config-responsibility-split` — none of the 23 enumerated ids appears in any INV-28 line. `check-state.py` exits 1, but on `VIOLATION .harness/harness/features/FEAT-34-worktree-act3-enforced/BRIEF.md is NOT approved`, not on anything from this feature; INV-28 is a `note`, so T-05's "do not change exit behaviour" holds. |
...
3. `pr: true` — the bool exclusion in `_record_pr` (`gh-sync.py:562`) and INV-28
   (`check-state.py:1078`) is unbound in both suites. Not an SC clause; a mutant dropping either
