...
## F1 (re-check) — schema-version monotonicity check embedded in `check-domain.py`'s `shape_problems`

- file/line: `.claude/skills/harness/bin/check-domain.py:1760-1779` (`_prior_version` /
...
- The concrete question this run was asked to settle: could this move to the schema
  ...
  *within* one instance, never across two files on disk. Expressing monotonicity requires
  ...
  already does two paragraphs above (the `prior_state`/`prior_doc` load `check-domain.py`
...
- Is this a special case bolted onto shared infrastructure, or the sweep's own business?
  ...
  to decide it; `check-domain.py` never needs this because it is a write-time gate acting
