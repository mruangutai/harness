...
`check-domain.py`, `check-state.sh`, `run-state-schema.json` and their tests operator-owned, so the
...
| B-16 ⌘ | chore | **Guards argued fail-closed, not mutation-proven** — `check-domain.py`'s step guard got six live probes at c11, but `validate-digest.py` and `check-state.sh`'s guards remain reasoned only; DEC-174 forbade the mutation that would demonstrate them | c9 panel, partial at c11 |
