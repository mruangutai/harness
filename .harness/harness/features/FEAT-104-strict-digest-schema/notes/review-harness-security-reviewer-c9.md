...
Scoped **IN**. Measured: read every hunk of `check-domain.py` (+102), `check-state.sh` (+54),
...
- **Pre-existing, newly checked, confirmed non-issue:** the `_no_parser` bootstrap-grant early return
  in `check-domain.py` predates this diff and is unconditionally caught by `check-state.sh`'s next
  sweep pass, same as before.
- Path arguments to the new schema loads (`check-domain.py`'s `sys.argv[3]`, `check-state.sh`'s
...
| run-state-schema.json path resolution (both scripts) | Tampering (path traversal) | true — BASH_SOURCE-derived selfdir in both check-domain.py and check-state.sh |
