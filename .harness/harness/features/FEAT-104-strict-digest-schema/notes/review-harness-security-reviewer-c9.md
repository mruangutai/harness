...
own audit surfaces **one new med finding**: `check-state.py`'s new INV-16 at-rest message echoes the
...
Scoped **IN**. Measured: read every hunk of `check-domain.py` (+102), `check-state.py` (+54),
...
`check-domain.py` now reads the **on-disk prior document** for every state.yaml write (the
...
it binds the blocking PreToolUse path, not only the post-hoc report. `check-state.py`'s at-rest sweep
...
- **Bypass hunt (per dispatch item 1), beyond the already-escalated Bash-write residue:** checked the
  `_no_parser` early-return in `check-domain.py` (fires when PyYAML is unavailable in a bootstrap-grant
  ...
  introduced by this diff, and is already covered by `check-state.py`'s at-rest sweep catching the
...
`check-state.py:1590` still calls `_vd_mod.validate("lead", _dtext)` with the **literal** string
`"lead"`, not `_host` (the real raw persona already held at `check-state.py:1577`) — unchanged by this
...
- **Residual, non-gating risk, stated explicitly:** the decline leaves the at-rest sweep blind to raw
  ...
  `check-state.py`, unlike the symmetric `state.yaml` step case (which the sweep does catch at rest,
...
## New — SEC-C9-01 (med, non-gating): unescaped run_id/step-id echo in check-state.py's INV-16 message
`check-state.py`'s new sweep (`:1483-1531`) builds, for every schema-invalid step on a
...
the pattern every other attacker-controlled value in the *same* diff follows: `check-domain.py`'s
sibling message uses `repr(key) for key in sorted(...)` explicitly; `check-domain.py`'s downgrade
message uses `{_version!r}` explicitly; `check-state.py`'s own `_names` is list-wrapped. Only `run_id`
...
- **Concrete failure scenario.** *Actor:* anyone who can place a `state.yaml` on disk without going
  through `check-domain.py`'s Write/Edit hook — the already-acknowledged, out-of-scope Bash-authored-
...
- **Why med, not high:** exploitation requires the precondition already carved out of this review's
  ...
  can now be selectively defeated. **Non-gating**, reported for the next touch of `check-state.py`; the
...
- **Pre-existing, already-escalated, not re-raised:** Bash-authored `state.yaml` writes bypass
  `check-domain.py` entirely (DEC-85's acknowledged sharp edge) — this is the precondition for
...
- **Pre-existing, newly checked, confirmed non-issue:** the `_no_parser` bootstrap-grant early return
  in `check-domain.py` predates this diff and is unconditionally caught by `check-state.py`'s next
  sweep pass, same as before.
- Path arguments to the new schema loads (`check-domain.py`'s `sys.argv[3]`, `check-state.py`'s
  `sys.argv[2]`) both resolve to `_selfdir`, derived from `BASH_SOURCE[0]` — the script's own
  directory, confirmed for **both** scripts this cycle (c7 verified only `check-domain.py`). No
  traversal in either.

## Message-exposure section
- Digest-key rejection (`validate-digest.py:1414-1421`): `repr(field)` for every offending key — safe.
- Step/evidence rejection (`check-domain.py`): `repr(key)` for every offending key — safe.
- Downgrade rejection (`check-domain.py`): `{_version!r}` explicit repr; `{_prior_version}` is
  type-constrained to a plain positive int by the `_prior_is_strict` guard before it can appear — safe.
- INV-16 at-rest rejection (`check-state.py`): offending-key list `_names` is list-wrapped (safe via
...
|---|---|---|
| agent-authored `state.yaml` write vs. check-domain.py schema_version floor/downgrade gate | Tampering | true — F1 fixed and executed |
| agent-authored `state.yaml` write vs. check-domain.py closed step schema | Tampering | true — unchanged from c7, still sound |
...
| Bash-authored `state.yaml` vs. check-state.py's INV-16 operator report | Spoofing / Information disclosure | false — SEC-C9-01, gated behind the pre-existing Bash-write precondition |
| run-state-schema.json path resolution (both scripts) | Tampering (path traversal) | true — BASH_SOURCE-derived selfdir in both check-domain.py and check-state.py |
...
## Open questions
- SEC-C9-01's one-line fix (`!r}` on `run_id`/`_step_id` in `check-state.py`'s INV-16 message) is
...
VERDICT: PASS
DIGEST:
  headline: "F1 (high) confirmed fixed at source and by execution (12/12 T-06); F3 (med) confirmed fixed at source and by execution (34/34 T-04); F2 (med) correctly declined — independently reproduced the REQ-08/SC-12 stranding evidence on a real digest and confirmed no schema_version-equivalent discriminator exists for digest.md to make the remedy safe. One new med, non-gating finding: check-state.py's new INV-16 message echoes run_id/step id unescaped, unlike every sibling interpolation in this diff, letting the pre-existing Bash-write bypass also spoof/erase its own audit line on the operator's terminal."
  in_scope: true
  ...
  must_fix: []
  threat_model:
    - { boundary: "state.yaml write vs. check-domain.py schema_version floor/downgrade", stride: T, mitigated: true }
    - { boundary: "state.yaml write vs. check-domain.py closed step schema", stride: T, mitigated: true }
    - { boundary: "at-rest digest sweep vs. raw persona (F2)", stride: T, mitigated: false }
    - { boundary: "Bash-authored state.yaml vs. check-state.py INV-16 report echo", stride: S, mitigated: false }
  ...
  open_questions:
    - { id: Q1, question: "SEC-C9-01: check-state.py's INV-16 message should wrap run_id/step id in repr() (or a list) to match every other attacker-controlled interpolation in this diff, closing a terminal-escape spoof of the audit line reachable via the already-accepted Bash-write bypass. DEC-174 carve-out, main-session route.", blocking: false }
