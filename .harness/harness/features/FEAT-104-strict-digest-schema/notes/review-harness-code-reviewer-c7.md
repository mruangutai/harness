...
- **REQ-01** (undeclared digest key rejected at every tier) — **MET.** The closed-set check is
  ...
  (`check-state.py:1590`) never exercises this check for lead digests, at any date.
...
- **REQ-05** (rejection names every key and its route) — **MET** at the digest tier
  ...
  at the step tier (`check-domain.py:1653-1657`, `check-state.py:1525-1529`), though the two step-tier
...
- **REQ-08** (historical artifacts unrewritten) — **MET, independently re-verified.** I ran T-10's own
  ...
  (`check-state.py:1483-1485` census comment, dated).
...
1. **Does the unknown-key rejection reach exit 2 on every tier, or is one still ignored?**
   ...
   there is a tier that is permanently blind to it**: `check-state.py:1590` calls
   `_vd_mod.validate("lead", _dtext)` for **every** completed run whose `host` is in `LEADS`
   (`check-state.py:1396`) — not only historical ones, and not gated by any date or
   ...
   current: `_host` (`check-state.py:1483`) **is** the real raw persona
   ...
   **Concrete failure scenario:** a lead is re-prompted with `stop_hook_active` set (the acknowledged
   ...
   resulting `digest.md` — now containing that key — is later swept by `check-state.py`'s INV-15 check
   ...
   not actually re-checked by the one mechanism (`check-state.py`) that exists specifically to catch
   drift that slipped past the hook. Remedy is passing `_host` instead of `"lead"` at
   `check-state.py:1590`, gated on some era marker so historical files stay exempt — that edit is
   inside `check-state.py`, a DEC-174 carve-out. **route: main-session.**
...
- **S2** (`check-domain.py:1653-1657` vs `check-state.py:1525-1529` — one shape, two hand-written
  ...
  caught only by `check-state.py`'s sweep, is told to "put per-dispatch facts under evidence" — which
...
- `run-state-schema.json`'s top-level `additionalProperties: false` is declared but never consulted
  by either gate (`check-domain.py`/`check-state.py` only load `properties.steps.items`) — inert
