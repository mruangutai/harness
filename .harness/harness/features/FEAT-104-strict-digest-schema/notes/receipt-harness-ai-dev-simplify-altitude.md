...
## F4 — `check-domain.py` and `check-state.sh` each hand-roll the same step-schema loader
...
- The two-enforcement-point *architecture* itself is not the problem: write-time refusal
  (`check-domain.py`) plus at-rest reporting (`check-state.sh`) is the established harness pattern
