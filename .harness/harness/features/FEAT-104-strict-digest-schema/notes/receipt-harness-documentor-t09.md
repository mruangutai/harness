...
- **DEC-223 — "The digest contract is closed, and the run-state step vocabulary with it"**
  ...
  rest by `check-state.py`, gated on `schema_version` 2 with creation below 2 refused and the 356
...
`check-domain.py:1586-1612` and the at-rest sweep gate at `check-state.py:1484-1492`; both
