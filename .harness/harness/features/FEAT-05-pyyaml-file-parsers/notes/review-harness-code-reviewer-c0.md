...
**check-domain.py, bash-write-guard.py, gh-sync.py, cost-report.py, upgrade-config.py — all match
...
- Fail-closed/deadlock: main session (`agent = ""`) exits 0 in both hooks **before**
  ...
  `harness-dev-ops` is exempt only from `bash-write-guard.py` (`:56-57`), not from
