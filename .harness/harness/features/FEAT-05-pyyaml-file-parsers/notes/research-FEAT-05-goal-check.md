...
**The blocker.** `check-domain.py:318-335` is a line-scan alternative selected on parser
...
`CLAUDE.md` says it a third time. `check-domain.py` is a converted script. The in-code comment
...
| SC-12 | automated | **met** | `run-unit-tests.py` at HEAD: **exit 0, 12 PASS, 0 FAIL, 0 MISCONFIGURED**, ≥ the 9-suite baseline. The five `skip` string matches are test-case *names* about gh-sync's SKIP behaviour; the runner emits no skip mechanism (`grep -n skip run-unit-tests.py` → nothing). |
...
by D-04, writers stay line-based) and `check-domain.py:319`'s key scan is new. **REQ-08 partial** —
see SC-14(b).

## What would settle the two partials

- **SC-03 / SC-04:** either remove `check-domain.py:318-335`'s scan, or amend the BRIEF with a D-NN
...
- **Doc drift the propagation checker did not catch.** `check-docs.sh` exits **0** (measured), while
  ...
  `check-domain.py:318-335`, those two sentences need amending with it.
