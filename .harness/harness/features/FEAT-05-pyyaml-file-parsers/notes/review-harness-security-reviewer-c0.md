...
The author's claim — "the main session is exempt at `check-domain.py`'s no-agent-identity
...
existing DEC-110-grade design note in `check-domain.py:9-11,70-73` asserting this is measured
...
header (`check-domain.py:13-14`, DEC-100): *"Only exit 2 blocks — exit 1 is a NON-blocking
...
- **Manifest path is a directory** (`os.access(manifest, os.R_OK)` at `check-domain.py:100`
  ...
  ```
  $ check-domain.py   (.harness/team-config.yaml is a directory, not a file)
...
`check-domain.py` exits at `:122-123` (`if harness_yaml.yaml is None: sys.exit(0)`) **before**
...
- **YAML 1.1 type coercion (`on:` → `True`)** — `check-domain.py:332-333` `str()`-coerces every
...
- **Marker file edge cases** — directory, unreadable, write-failure all route through
  ...
  universal read-only `.` entries — which `check-domain.py`'s own `matches()` explicitly
