...
```sh
CHECK_STATE_BIN=<isolated-bin>/check-state.py python3 -c "import importlib.util as u, sys; s=u.spec_from_file_location('t','tests/integration/test-check-state.py'); m=u.module_from_spec(s); s.loader.exec_module(m); sys.exit(0 if m.case_bug440_digest_verdict_reconciliation() else 1)"
```

Verbatim failing output against the pre-change `check-state.py`:
...
RESULT: RED - case_bug440_digest_verdict_reconciliation() returned False against the pre-change check-state.py
