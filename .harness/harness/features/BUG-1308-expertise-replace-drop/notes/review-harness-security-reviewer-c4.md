...
`expertise-merge.py`, `harness_merge.py`, `check-expertise.py`, and both named test files.
...
named test files (+81/+28) — `harness_merge.py`/`check-expertise.py` are byte-unchanged since c3.
...
refused colon-prefix attempt planted nothing. Final file passes `check-expertise.py` clean (`OK`).
...
| **unicode decimal digits** | `P-٠١` (Arabic-Indic), `P-०१` (Devanagari) | **exit 0**, ADDED, distinct id from `P-01`, round-trips, `check-expertise.py` OK |
| **fullwidth-digit homoglyph** | `P-０１` (U+FF10/11 — visually near-identical to `P-01`) | **exit 0**, ADDED as a DISTINCT id, `check-expertise.py` reports **OK** — see F1 |
...
| **oversized target** | `target = "P-" + "9"*10000` (10 KB id) | **exit 0**, ADDED, `check-expertise.py` OK — see F2 |
| **oversized entry, no whitespace** | `entry = "A"*10000` (one "word") | **exit 0**, `check-expertise.py`'s `WORD_CAP=50` never fires (counts words, not chars) — see F2 |
...
- **F1 (low, bug):** `_validate_target_grammar`'s round-trip through `ENTRY_RE` inherits Python
  ...
  ASCII `[0-9]`.
- **F2 (med, bug):** no character-length cap exists on `target`, and `check-expertise.py`'s
  ...
  DEC-145's size intent. Pre-existing in `check-expertise.py` (untouched this delta); newly and
