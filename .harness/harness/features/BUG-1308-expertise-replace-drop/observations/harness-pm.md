...
- 2026-09-05: BUG-1308 panel c2 — `check-state.py` INV-32 expects three readers (`should-not-exist`, `scope`, `goalcheck`) but a validator-lead digest for a plan panel names only the two validator-segment readers; the goal-check runs in the product segment. Transcribing what the lead recorded leaves INV-32 unsatisfiable by pm.
...
- 2026-09-05: plan-merge set-panel REPLACES the whole panel mapping from --value-file, so the safe
  ...
  silently dropped.
- 2026-09-05: INV-32 (check-state.py:533) names THREE readers - should-not-exist, scope, goalcheck -
...
- 2026-09-05: goal-check probes against expertise-merge.py must place the fixture at <root>/.harness/expertise/<agent-name>.md and title line 1 '# Expertise — <agent-name>': a temp path outside that shape hits the exit-9 'not an Expertise file' guard and a title/filename mismatch reddens check-expertise.py — both read as feature defects (O-06). Also: entry: carries the TEXT only; the tool renders '- <target>: <entry>'.
