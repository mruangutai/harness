...
> …the ownership condition scopes repository-wide enforcement to a checkout whose own index carries this predicate at that exact relative path, and `violations()` has exactly one caller — Harness's own `run-unit-tests.py` — so a product checkout is reached by neither the clause nor the runner.
