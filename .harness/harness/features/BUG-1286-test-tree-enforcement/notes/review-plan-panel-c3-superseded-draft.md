...
| 2a | scope | med | D-05's accepted archival landmine | Archiving FEAT-44's evidence file reddens every `run-unit-tests.py` invocation repo-wide until a backend-dev/dev-ops edit, though the archiver need not hold that grant |
...
- **Falsification evidence exists, so this is not a shallow pass:** `should-not-exist` verified at
  source that `run-unit-tests.py:33` already calls `violations()` on every invocation, which is what
