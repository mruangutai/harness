...
This is the canonical digest for run `2026-09-04-13-validator`. It supersedes the sibling
`digest.md`, whose only defect is a contract one: it carried `status: ran` on both member rows
(the schema permits `status` only as `skipped`) and omitted the `artifact:` scalar. The write guard
refuses an in-place replacement of `digest.md`, and the dispatch forbade spending a second run
directory, so the corrected record lands beside it under a distinct name. No finding, severity,
reader disposition or assessment differs between the two files.
...
DIGEST:
  ...
  open_questions:
    ...
    - { id: Q2, question: "harness defect, non-blocking: validate-digest rejects a member row carrying status: ran, while the team skill's own reporting template shows a member row with status. A lead that follows the template writes a file it cannot then repair, because the digest write guard refuses replacement - the only routes left are a second run directory or a sibling filename, and this run took the sibling.", blocking: false }
  escalations: []
  expertise_update: []
  sc_status: []
  adequacy_notes:
...
artifact: /Users/molchairuangutai/GitHub/harness/.claude/worktrees/harness/BUG-1286-test-tree-enforcement/.harness/harness/features/BUG-1286-test-tree-enforcement/runs/2026-09-04-13-validator/digest-final.md
```
