...
.claude/skills/harness/bin/check-state.py`). Judging text only, per dispatch: the rewritten INV-6
message, the new SKILL.md step 6 instruction, and the two new test cases' failure diagnostics.

## 1. INV-6 violation message (`check-state.py:462-464`) — cycle 0's MED is genuinely closed
...
can now act (`code_grade: n_a`) without opening `check-state.py` or `feature-schema.json`, closing
cycle 0's finding exactly as its own "one-line, reversible fix" suggestion described.

**Convention comparison against INV-32 (`check-state.py:286-297`).** INV-32's pattern is
...
`check-state.py` line 132 of this same SKILL.md calls the Build-phase QA step itself "a
...
but `check-state.py`'s `code_reviewing_runs` filter (unchanged in scope by this remedy: it only
...
recorded, `check-state.py` invoked, before the step-4 pin — INV-6 fires on ordinary Build-phase
...
is. **I could not confirm this manifests**: `check-state.py` is a self-run pre-commit discipline,
