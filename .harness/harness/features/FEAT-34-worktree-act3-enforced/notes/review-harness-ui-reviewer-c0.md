...
DESIGN.md in the diff. Surface is shell (`check-state.sh`, `post-merge-sweep.py`,
...
text added to `check-state.sh` and the `post-merge-sweep.py` stdout messages. Both are
