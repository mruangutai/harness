...
- check-state.py exits 0 with zero VIOLATION lines — run as `bash -c "cd <worktree> && ..."` — verified-at 9165162
- check-plan-routes.py exits 0, `0 violation(s)`, exactly 4 DEVIATION lines (T-06/T-08 on check-state.py, T-07/T-09 on test-check-state.py) — verified-at 9165162
...
- Do not run check-state.py or check-plan-routes.py from the default Bash cwd — they resolve `.harness/` against the working directory and silently scan the MAIN checkout, which produced one wrong conclusion in this phase already.
