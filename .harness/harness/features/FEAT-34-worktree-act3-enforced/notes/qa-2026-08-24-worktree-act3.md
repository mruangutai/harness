...
point that produced the three prior escaped defects. But the gate itself (`check-state.py`'s new
...
| `check-state.py` | 0 | — | 0 `VIOLATION` lines |
...
Where the vantage-point defense does NOT reach: `check-state.py`'s own INV-29/INV-30 are called
only from the repository root in every real invocation I ran (`python3 check-state.py` from the main
...
| SC-04 | second-repo predicate covered at library level, `case_second_repo` + `case_classify_all_two_repos`. One-`check-state.py`-run assertion untested — no `test-check-state.py` case exists. | T-07 pending |
...
| SC-10 | **not gradable as specified** — dispatch says grade the working tree, not `review_sha`; `check-state.py` clean and `integration` kind green on the working tree stands in. | review_sha not yet pinned, by design |
| SC-11 | **satisfied.** `case_per_feature_record` (d) — two separate milestone-number assertions (801, 802), never a total count. | — |
| SC-12 | **none.** `INV-30` occurs 0 times in `check-state.py` — T-08 unbuilt. | T-08/T-09 pending |
...
artifact (`check-state.py`'s INV-29 addition) NO test file in either bucket exercises it. That is a
