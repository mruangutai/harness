...
- 2026-08-24: FEAT-34 T-02 rework (classify-from-linked-worktree hole) — every existing case in
  ...
  root itself set to a linked worktree, which is how check-state.py/post-merge-sweep.py always call
...
- 2026-08-24: FEAT-34 T-01 rework — classify()'s main-checkout skip compared os.path.realpath(path)==os.path.realpath(root), which fails silently (fail-open into an unresolved/misclassified record, not a crash) whenever root is itself a linked worktree, because the main checkout's porcelain entry then never equals root. Fix: derive the main checkout from porcelain ORDER (index 0, per check-state.py's INV-25 precedent at :1138-1143) rather than a value comparison to a caller-supplied argument — the same class of bug as comparing a lookup's absence to a caller's identity instead of to the data's own known-total shape.
- 2026-08-24: FEAT-34 T-03/T-04 second rework — a fixture proving "wrong copy shipped" beats a fixture proving "SKIP happens": building a linked-worktree fixture where BOTH the main checkout and the divergent worktree have a same-id feature dir (different milestone numbers, e.g. 810 vs 811) turns the assertion on which milestone got PATCHed closed, never on presence/absence of a SKIP line — and it reproduced the operator's exact escalation (existing-but-wrong feat_dir, no SKIP at all) on the very first RED run.
