...
1. `env -u HARNESS_AGENT_TYPE bash .claude/skills/harness/bin/run-unit-tests.py --kind all`
...
2. `env -u HARNESS_AGENT_TYPE bash .claude/skills/harness/bin/run-unit-tests.py --check-kinds`
   → **exit 0**, prints `check-kinds: the script arrays and test_kinds.integration.detect agree.`,
   runs no test.
3. `env -u HARNESS_AGENT_TYPE bash .claude/skills/harness/bin/run-unit-tests.py --kind bogus`
