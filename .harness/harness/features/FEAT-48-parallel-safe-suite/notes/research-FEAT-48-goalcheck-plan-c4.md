...
- Also re-derived and **HOLDING**: `run-unit-tests.py` serial loop at **147-157** exactly (`for s in "${SCRIPTS[@]}"` at 148, `done` at 157); the literal `"${SCRIPTS[@]}"` occurs **exactly once**; drift detector `for s in` at **64**. The rebase did not touch this file.
