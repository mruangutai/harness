...
Every criterion's evidence sits on one of those files or on `run-unit-tests.py` (unchanged in range but
whose script sets changed), so I re-took all ten rather than carry any c8 figure across the rewrite.
`run-unit-tests.py` is byte-identical in range. Every suite invocation used `env -u HARNESS_AGENT_TYPE`;
...
|SC-04|**MET**|automated / unit|`--kind unit` exit 0, 33 files, 33 blocks, zero `FAIL`, emits `PASS test-suite-independence.py`; `pool: 8 workers, 33 files, 13.75s wall`. File is in `UNIT_SCRIPTS` (`run-unit-tests.py:30`)|re-taken at 27f8105b|
...
- **The ten-run fence should paste each run's own `pool:` line** (a T-06-shaped note edit), so a reader
  ...
  not reach it.
- **SC-07's failing-file clause remains composed, not gated** — no test drives `run-unit-tests.py`
