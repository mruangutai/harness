...
`bash .claude/skills/harness/bin/run-unit-tests.py --kind unit` — exit 0, **16/16 scripts PASS**:
```
...
`bash .claude/skills/harness/bin/run-unit-tests.py --kind integration` — exit 0, **12/12 scripts PASS**:
```
