...
- 2026-09-07: the panel disposition vocabulary is open|resolved. `closed` is not a token: check-state.py:527-533 treats anything not `resolved` and not an accepted risk as still open, so a `high` cycle-0 entry marked `closed` becomes a HARD bad — worse than the staleness being removed. resolved_by is genuinely optional; its absence produced no finding.
