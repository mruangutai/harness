...
- 2026-09-07: BUG-124 panel c1 transcription. check-state.sh INV-32 cannot reach a worktree-hosted, pre-signature plan twice over: it globs the OWNER checkout's .harness/*/features/* (:118-120) and grades only approved plans (:428-471). A full run from inside the worktree printed 867 lines, none naming BUG-124. Evidence route that worked: extract the INV-32 body (lines 473-555) verbatim, wrap it in a one-iteration for so its `continue`s are legal, and exec it with a frame of {doc, feat, approval, bad, warn, re} — 0 bad, 7 warn. Beats re-implementing the invariant.
- 2026-09-07: plan-merge.py set-panel takes ONLY a --value-file and validates just four keys and their types (last_run:str, cycle:int, readers:list, findings:list — plan-merge.py:1017-1037); it then safe_dumps and refuses if the block does not reload as supplied (:1062-1065). Staging the value with a python script that LOADS the existing plan and copies the prior finding dicts is what makes "carried verbatim" provable rather than retyped.
- 2026-09-07: the panel disposition vocabulary is open|resolved. `closed` is not a token: check-state.sh:527-533 treats anything not `resolved` and not an accepted risk as still open, so a `high` cycle-0 entry marked `closed` becomes a HARD bad — worse than the staleness being removed. resolved_by is genuinely optional; its absence produced no finding.
