...
## Task
Apply the ONE authorized SIMPLIFY finding (SIMPLIFICATION angle, `dispatch-guard.py:158`):
...
## Change
File: `.claude/skills/harness/bin/dispatch-guard.py`, BUG-124 T-02 run-dir shape check block.
...
```
git diff --stat -- .claude/skills/harness/bin/dispatch-guard.py
 .claude/skills/harness/bin/dispatch-guard.py | 47 ++++++++++++++--------------
 1 file changed, 24 insertions(+), 23 deletions(-)
```

```
git status --porcelain
 M .claude/skills/harness/bin/dispatch-guard.py
...
```
Only `dispatch-guard.py` was modified by me; the four `??` receipts belong to sibling SIMPLIFY
