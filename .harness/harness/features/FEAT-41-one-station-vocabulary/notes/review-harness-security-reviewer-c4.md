...
**Live confirmation, `select_base`/`classify` via `bash-write-guard.py` (HIGH):**
...
|---|---|---|---|
| same command, **no NUL**, same agent | `bash-write-guard.py` | 2 | `BLOCKED — …redirect targets .claude/skills/harness/bin/check-domain.py, outside your domain.` (correct) |
| same command, **with NUL** | `bash-write-guard.py`, `origin/main` | 1 | Traceback, `ValueError: lstat: embedded null character` (pre-existing crash, same as (a)) |
| same command, **with NUL** | `bash-write-guard.py`, `64f42ef8` | **0** | **(empty — no denial, no message)** |
...
`{"outcome": "not_a_domain_question", "base": None, …}`, and `bash-write-guard.py:747` treats that
...
such layer** — `bash-write-guard.py` never routes through `_plan_route`; it is check-domain.py's own
...
MF-3/T-19 were about. A raw Bash write via any method `bash-write-guard.py`'s classifier does not
...
2. **`harness_boundary.py:288` `select_base()`'s `inside()` check (`:352-374`)** — `real()`'s
   ...
   `bash-write-guard.py` has no independent safety net for this (unlike `check-domain.py`'s
...
- None blocking. Whether (2) is fixed by making `real()`'s fallback itself resolve as much as it
  safely can (fixing it once, centrally) versus giving `bash-write-guard.py` its own
...
VERDICT: FAIL
DIGEST:
  headline: "Job (a) settles cycle 3's attribution as wrong (crash is pre-existing, this feature fixed it); jobs (b)/(c) find the fix is sound for check-domain.py but leaves bash-write-guard.py silently permitting an out-of-domain Bash write on an unresolvable path; and a live, mutation-proven new HIGH — station_only:true forged onto a task-bearing plan silently disables both INV-34 checks."
  in_scope: true
  ...
  open_questions:
    - { id: Q1, question: "Fix (2) by making harness_boundary.real()'s fallback itself resolve as far as safely possible (central fix), or give bash-write-guard.py its own unconditional-refuse-on-unresolvable layer matching check-domain.py's _plan_route (duplicated fix)? Design call for whoever fixes it.", blocking: false }
  files_touched: []
  expertise_update: []
