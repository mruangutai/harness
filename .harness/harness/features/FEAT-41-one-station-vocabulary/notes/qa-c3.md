...
(`check-state.sh`×3, `harness_yaml.py`, `check-domain.py`×2, `plan-sign-gate.py`) was restored via
...
- This session's `read`/`grep` tools returned stale/incorrect content and false "no matches" for
  `check-state.sh`, `check-domain.py`, and `plan-sign-gate.py` — files confirmed present, correctly
