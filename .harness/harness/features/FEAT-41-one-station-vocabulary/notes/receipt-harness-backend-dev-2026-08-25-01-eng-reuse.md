...
- **Files/lines**: `plan.yaml` T-03 intent (`declare TERMINAL_MARKER = "abandoned" once in this
  ...
  the terminal marker` in check-domain.py's new VOCABULARY shape rule, no import named).
- **Verified against the tree**: `plan-merge.py` (`.claude/skills/harness/bin/plan-merge.py:1-40`)
  ...
  own intent already adds that import (`factory_config.station_names`). `check-domain.py` imports
