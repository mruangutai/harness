...
| `check-omp-port.py` | **session-entry**, not hot | Called only from `check-state.py:972-977`; `check-state.py` is invoked once per door by `.claude/commands/harness.md:12` ("Run `check-state.py`"), which `harness-plan.md`/`harness-ship.md`/`harness-grilling.md` all route through |
