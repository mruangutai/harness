...
| A4 corpus count | `test-harness-yaml-corpus.py` | `ok    every shipped YAML parses (101 files across 2 roots: .harness=97, .claude/skills/harness/teams=4)` | at a CLEAN checkout of pin `50dd75c4`: `ok    every shipped YAML parses (103 files across 2 roots: .harness=99, .claude/skills/harness/teams=4)` | the corpus is every tracked `*.yaml`/`*.yml` under `.harness/` (JSON is not counted). `git diff --name-status a4a3d7f8 50dd75c4` adds exactly two: `…/FEAT-64-broad-exception-libs-tools/plan.yaml` and `…/runs/validate-validator/state.yaml` (the c0 panel's committed state). Each later committed `runs/validate-cN-validator/state.yaml` raises the count by one more; the number recorded in `byte-evidence.md` is the one measured at a clean checkout of the sha its header names. (validate c1 GC-64-04: the earlier text here named `feature.json` and an evidence run made in a working tree that also held the untracked `runs/plan-product/state.yaml` reported 104/100 — both corrected from the recorded delta.) |
