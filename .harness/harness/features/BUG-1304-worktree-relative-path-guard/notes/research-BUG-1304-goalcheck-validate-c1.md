...
`check-domain.py:770-816`, `bash-write-guard.py:737-761`, `inflight_registry.live_claims`). REQ-07 is the
...
- **SC-05 — unproven, not wrong. Remedy is TEST.** `claim_set_refusal` always emits
  ...
  `bash-write-guard.py:757-761`), so the message is correct — only the assertion is missing. Fix is one
...
- **SC-07 — unproven and unwritten, not wrong. Remedy is RECORD.** The ambiguous-claim treatment is
  implemented (`check-domain.py:779-784`, `bash-write-guard.py:738-742`), tested (SC-05 ambiguous
