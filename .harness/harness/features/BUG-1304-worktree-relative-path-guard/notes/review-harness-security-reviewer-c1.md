...
`bash-write-guard.py` behaviour tests below therefore evidence `af5ddd7a`'s own behaviour, not a
later edit).

## Headline

`bash-write-guard.py`'s write-pattern detector does not recognize a bare `python3
...
## Finding 1 (HIGH, must_fix) — claim registry is a first-class attack surface bash-write-guard.py does not see
...
Verified live against the pinned script (`bash-write-guard.py:af5ddd7a`, byte-identical to
...
sys.exit(0)` (`.claude/skills/harness/bin/bash-write-guard.py` — the `findings` build block,
...
`claim_checkout_guard`) / `if not claim_set: return []` (bash-write-guard.py `claim_checkout_guard`).
...
agent from choosing to run it, and `bash-write-guard.py` — the artifact whose entire stated purpose
...
**Anchors (af5ddd7a):**
- `.claude/skills/harness/bin/bash-write-guard.py` — finding-pattern block (redirect/sed/tee/mv/cp/rm/sponge/awk/git-worktree only) and the `if not findings: sys.exit(0)` short-circuit ahead of the reviewer deny and the domain walk.
...
**Anchors:** `.claude/skills/harness/bin/harness_boundary.py:claim_worktrees` (`worktree_for_feature(owner_root, claim.get("feature"))`, `worktree_for_feature`'s `feature_id.startswith`), `check-domain.py:claim_checkout_guard` and `bash-write-guard.py:claim_checkout_guard` (`except Exception as exc:` blocks).
...
VERDICT: FAIL
DIGEST:
  headline: bash-write-guard.py's write-pattern detector does not see `inflight_registry.py release`/`release-all` over Bash, so any governed agent (incl. read-only reviewers) can empty or wipe the claim registry BUG-1304's whole binding decision depends on, with zero denial — verified live, exit 0 both ways
  in_scope: true
  ...
  findings: 2
  must_fix:
    - "bash-write-guard.py's finding detector must treat `inflight_registry.py {release,release-all}` (and any other direct mutator of `.harness/.inflight-claims.json`) as a write, gated at least as strictly as domain/claim enforcement — otherwise claim-set binding is opt-out for any governed agent over Bash. Anchors: bash-write-guard.py finding-pattern block; harness_boundary.py:claim_worktrees `if not claim_set: return`; inflight_registry.py release/release-all."
  threat_model:
  ...
  open_questions:
    - { id: Q1, question: "Should bash-write-guard.py's finding detector be extended to recognize inflight_registry.py CLI invocations (release/release-all/reconcile) as writes needing at least the same claim-checkout scrutiny as any other governed write, or should claim-registry mutation be restricted to a non-Bash-reachable seam entirely (e.g. only invoked in-process by dispatch hooks, never as an agent-issued CLI)?", blocking: true }
  files_touched: []
  expertise_update: []
