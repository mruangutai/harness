...
- C7 DONE — `add-tasks` T-09 and T-10. T-09 = OC-1, STRIKEABLE AT SIGNATURE, carrying both halves
  ...
  G-1a broad verification, `run-unit-tests.py --kind all` plus `check-state.sh`, depends only on the
...
| G-1a — nothing verifies wider than the suite it edits | New task T-10: `run-unit-tests.py --kind all` then `check-state.sh`, both exit 0. Not bolted onto any existing `verify:`. |
