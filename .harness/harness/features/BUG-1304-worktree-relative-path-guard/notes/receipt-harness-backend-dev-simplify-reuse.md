...
`claim_checkout_guard` and `bash-write-guard.py`'s `claim_checkout_guard` both compute
...
is precedented, not new tech debt.** `check-domain.py:770` and `bash-write-guard.py:729`'s new
...
only in `deny(...)` (bash-write-guard.py, its established host-local reporter) vs
...
(check-domain.py:737, bash-write-guard.py:699), which this new function sits directly beside and
