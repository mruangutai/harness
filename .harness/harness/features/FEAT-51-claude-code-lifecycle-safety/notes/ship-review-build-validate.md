...
| **B-15** | **chore** | The orphan-refusal MESSAGE is hand-written twice — `check-domain.py:1695-1702` and `plan-sign-gate.py:400-414`, character-identical middle sentence — while the predicate is correctly single-homed. `plan-sign-gate.py`'s own header states the one-refusal-text rule the same file breaks 350 lines later. |
