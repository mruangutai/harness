...
`check-domain.py`, `plan-sign-gate.sh`, `quarantine.py adopt`, the whole word `Bash`, and one
...
It now requires **both** script names, `check-domain.py` and `plan-sign-gate.sh`, plus one sentence
