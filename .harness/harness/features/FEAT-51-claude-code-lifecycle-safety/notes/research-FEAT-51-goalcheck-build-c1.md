...
`check-domain.py` after the batch contract was measured, so both whole suites were re-measured here
...
| 04 | automated | `/tmp/gc51_probe_sc0405.py` — real `check-domain.py`, real registry. (1) orphan `Write` of `BRIEF.md`; (2) same write with the writer's own claim live; (3) orphan `Write` of `plan.yaml` | (1) exit 2, stderr names the **exact** path `…/quarantine/harness-orchestrator-feat51-w/BRIEF.md`; (2) exit 0; (3) exit 2 carrying `exactly ONE writer` and **no** quarantine text | **met** |
...
- **OMP allow at registry AND hook level, with a live supervisor pid.** Behaviour ✅ — registry
  `orphan_write` returns `False` and `check-domain.py` exits 0. Committed guard: registry side is
