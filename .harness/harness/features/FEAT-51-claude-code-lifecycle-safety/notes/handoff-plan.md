...
- F-A is real: `agent_type: harness-pm`, no live claim, `cp /tmp/evil.md <feature>/BRIEF.md` exits 0 at `bash-write-guard.py`, `plan-sign-gate.sh` AND `check-domain.py` — I fired all three payloads myself — verified-at 0bc57c88
...
- Do not use a relative path in any `rm`/`cp` under `bash-write-guard.py`; it resolves against the MAIN checkout and refuses with a message that reads like a domain error — measured twice in earlier cycles — UNVERIFIED here
