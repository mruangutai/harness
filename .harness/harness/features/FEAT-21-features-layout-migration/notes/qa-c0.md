...
| T-05 | logic | check-state.py | test-check-state.py | INTEGRATION | **no** |
...
Scratch copy: `/private/tmp/.../scratchpad/bin-probe/` (copied from `.claude/skills/harness/bin/`, outside the repo). Both probes run the **real, unmodified** `test-check-state.py` against the mutated `check-state.py` via `CHECK_STATE_BIN` env override (the file's own documented escape hatch, `test-check-state.py:14-18`).

**(a) delivery half — neutered `fpath()` to bare basename.** Mutation diff confirmed applied (`diff` shown against the real file). Ran full suite: **exit 0, 0 FAIL lines** — every case still `ok`. **Finding: nothing in the suite asserts the label content `fpath()` produces.** Grepped `test-check-state.py` for any assertion naming a segment-qualified path (`.harness/harness/features/FEAT...`) — none exists outside the fixture-builder's own docstring comment (line 41), which is prose, not an assertion. Measured (not just reasoned) that the label is correct today: re-running `check-state.py` over the live tree (`cs-out.txt`, captured earlier this run) shows the finding text `.harness/harness/features/FEAT-19-central-product-config/plan.yaml approval is pending — awaiting the user.` — segment-qualified, as D-08 intends. So the delivery half is measured-correct today but pinned by no test; a regression that reverts the label to a bare basename ships silently.
...
| SC-03 | `check-state.py` re-run at pin: exit 0, 0 INV-27 lines |
...
- `check-state.py` zero-discovery regression: unbound on the real tree (Job 3). New finding.
