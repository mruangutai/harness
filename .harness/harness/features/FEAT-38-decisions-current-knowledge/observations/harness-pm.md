...
- 2026-08-29: T-29 argv class audit. The plan's intent said the enumeration returns 72 files at 99bb52c; after T-24's deletions the current tree returns 70. Re-deriving the figure was cheap and the note would have carried a false count otherwise (P-14 held).
- 2026-08-29: the enumeration pattern is far wider than real execution: 17 of 70 candidates matched only in docstring prose or in attribute rebinds (test-factory-gh.py has 92 hits, ALL of them 'fgh.subprocess.run = fake' assignments and zero call sites). Grepping for 'subprocess.run(' WITH the open paren separated call sites from mentions in one command.
- 2026-08-29: REQ-10's class is not empty — 11 of 70 build argv from a parsed value. gh-sync.py:775 is the sharpest (BRIEF.md and plan.yaml prose into 'gh issue create --title/--body'). Nine others are just a repo slug or fleet.yaml default_branch reaching a fixed executable's argv, which is a much weaker risk and I said so rather than flattening the two into one finding.
- 2026-08-30 (FEAT-38 goal-check at 635cd3ba): a dispatch told me to grade SC-13 `met` "citing the standing UAT" and to only show my own measurement if I disagreed with the void-condition math. The void math was right; the PREMISE was wrong — no operator pass existed anywhere. `notes/uat-FEAT-38.md` was `status: ready` with all four `result:` blank, both prior goal-checks and both CEO briefings said "unrun", and ruling 6's "the judgement STANDS and is not re-run" POSTDATED those briefings, so it asserted a judgement that had never been made. Two live artifacts (`STATE.md:22`, a T-27 receipt) had already inherited the false premise. The instruction to re-derive was aimed at the arithmetic, not at the existence claim behind it — and the existence claim was the one that was false.
- 2026-08-30 (FEAT-38): SC-04's pinned "30 and 24" reproduced EXACTLY only once I also excluded `.harness/harness/docs/DECISIONS*` from the sweep — the criterion's stated exclusion list named three prefixes and not the authority docs themselves. Its third figure ("37" for am.N) reproduced under no invocation (34 occ / 31 lines under that same exclusion, 92 / 67 with the docs in). Two pinned counts in one sentence, measured under two different scopes, one of them unrecoverable.
- 2026-08-30 (FEAT-38): the UAT script's own header pinned a STALE review_sha (48bbe7e) and its stated entry length for DEC-181 (51 lines) was wrong at the real pin (46) because T-27 later removed that entry's three markers. A uat artifact written at one pin and consumed at another rots in exactly the numbers the operator uses to check they are reading the right text.
- 2026-08-30: FEAT-38 uat-FEAT-38.md `tree:` header pinned review_sha 48bbe7e and justified itself with a b32013c/48bbe7e byte-identity argument; the pin moved to 635cd3ba and the argument rotted while every sentence stayed true-looking. A UAT header that names a sha needs a re-derive step whenever the pin moves, and a stated line count beside a command is a second rottable anchor (DEC-181 51 -> 46 lines).
