...
| 15 | **met** | automated | `run-unit-tests.py@pin:31` `INTEGRATION_SCRIPTS` does not name the removed checker or its test; `check-decision-claims` absent from the whole file. `harness.json@pin` `test_kinds.integration.detect` likewise. Runner half: exit 0, 0 `FAIL` lines (SC-10's captured run) |
...
| 18 | **met** | automated | Both paths **byte-identical** to `git show 99bb52c:` — sha256 `adb9a648cfd167e3…` (7561 B) and `7a4e0ba1afcb20b4…` (12677 B), re-derived here, matching the panel's figures. `test-check-decision-anchors.py` named by `run-unit-tests.py@pin:31` **and** by `harness.json` `integration.detect` |
