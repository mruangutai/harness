...
| 10 | met | automated | `out=$(python3 .claude/skills/harness/bin/run-unit-tests.py 2>&1); ec=$?` — captured, never piped to `tail`. exit=0, `^FAIL` lines = **0**, `^PASS` lines = **1117**, `^ok - ` = 184, 3405 lines total, 175 s. 1117 matches the orchestrator's figure exactly |
