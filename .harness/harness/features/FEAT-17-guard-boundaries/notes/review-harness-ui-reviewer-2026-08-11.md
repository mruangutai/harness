...
- **Existing convention already governs the shape.** Confirmed by grep: `check-domain.py` and
  `bash-write-guard.sh` both follow one actionable-rejection pattern (`check-domain.py:641`,
...
against runtime convention at `check-domain.py:219,268,660` and `bash-write-guard.sh:324,353,364`) are
