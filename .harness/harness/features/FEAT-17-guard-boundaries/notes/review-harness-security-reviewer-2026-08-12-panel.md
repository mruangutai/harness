...
`classify()` at `harness_boundary.py:271-276` (`_wt_owner is not None and not _wt_owner[2]` — `None` short-circuits false, falls through to `not_a_domain_question`, i.e. **allow**), `check-domain.py:420-421`, `bash-write-guard.py:128-129` (same pattern, root-side).
...
- `{ id: Q2, question: "Does the new worktree-creation scan (bash-write-guard.py:365-431) resist invocation via 'sh -c', 'command git', a shell alias, or xargs? Not tested here.", blocking: false }`
