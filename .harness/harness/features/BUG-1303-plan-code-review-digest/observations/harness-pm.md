...
- 2026-09-05: BUG-1303 SC-02 said "both are named cases in the suite output" but the suite emits ONE ok line naming both directions. Graded met because each direction is a separate conjunct of _discrimination_ok over the same helper the live grading uses, so neither is ever-green — the criterion's stated purpose. A one-line report costs diagnosability, not falsifiability; when authoring, say "one report line per direction" if you mean two lines.
