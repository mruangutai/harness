...
|---|---|---|---|
| `check-domain.py:2058` | PRE, Edit on run-identity path | `_claimed_abs(target)` | No — guarded by `and target` at :2056 |
| `check-domain.py:2087` | PRE, Edit on digest/state-yaml/handoff path | `_claimed_abs(target)` | No — guarded by `and target` at :2059 |
| `check-domain.py:2092` | PRE, Write | `_claimed_abs(target)` | No — reached only when `_tool == "Write" and target` (the `elif _tool != "Write" or not target: sys.exit(0)` at :2088 has already exited otherwise) |
| `check-domain.py:2109` | POST, named file | `_claimed_abs(target)` | No — reached only inside `elif target:` at :2094 |
| `check-domain.py:2232` | POST, Bash sweep (no named file) | `_p` from `_glob.glob(_pat)` | No — glob only yields paths to files that exist on disk |

`_claimed_abs` (check-domain.py:334-351) also can't collapse a truthy `target` to a falsy return: `os.path.isabs(path)` is `False` for a non-empty relative string, and the join branch `os.path.join(root, path)` stays non-empty for any truthy `path`.
