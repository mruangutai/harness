...
| MF-8 | **Addressed.** `BRIEF.md` prose corrected to **three** (`check-state.py`, `check-domain.py`, `validate-digest.py`; `bash-write-guard.py` untouched) and states the carve-out is not widened — DEC-174 already names four. The disposition table left alone. |
