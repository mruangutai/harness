...
- All four project gates are green together — `validate-feature-json.py` rc 0 over 17 files,
  `check-plan-routes.py` `0 violation(s) across 10 plan(s)`, `check-state.py` rc 0 with zero
  violations, full `run-unit-tests.py` rc 0 — verified-at 3abaedd, run by me, not relayed.
- INV-17 emits exactly one exemption note, for FEAT-15, naming three suppressed stems — `check-state.py` output — verified-at 3abaedd.
...
- Do NOT chase `check-plan-routes.py` or `check-state.py` red — both went green at T-08 — verified-at 3abaedd.
...
- Do NOT edit the four DEC-174 carve-out files — `check-domain.py`, `bash-write-guard.py`,
  `validate-digest.py`, `check-state.py` — CLAUDE.md — verified-at 3abaedd.
