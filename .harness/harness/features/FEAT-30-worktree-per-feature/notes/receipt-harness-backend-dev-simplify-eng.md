...
- **File/line:** `.claude/skills/harness/bin/check-expertise.py:39`, `CAPS = {"Patterns": 15,
  ...
  the same constant `check-expertise.py` already enforces.
- **Cost:** DEC-145's four cap numbers now live in two places. `check-expertise.py`'s copy is
...
- **Alternative:** either (a) have `expertise-merge.py`'s cap check shell out to
  `check-expertise.py` after the union write instead of re-asserting the cap itself, or (b) extract
  `CAPS` into a small importable module (e.g. a `expertise_format.py` sibling) that both
  `check-expertise.py`'s heredoc and `expertise-merge.py` import. Either removes the second
