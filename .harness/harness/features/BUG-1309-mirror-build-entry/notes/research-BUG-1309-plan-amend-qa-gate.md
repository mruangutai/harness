...
- control: `post-merge-sweep.py`, `check-state.sh` → harness-backend-dev, harness-dev-ops
...
membership**: the same resolver grants team on `post-merge-sweep.py` and `check-state.sh` themselves
