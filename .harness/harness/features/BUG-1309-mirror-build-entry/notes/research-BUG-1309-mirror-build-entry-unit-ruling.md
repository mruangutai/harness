...
and are fixed in place: the retention test is at `post-merge-sweep.py:228`, not `:222`, and the
...
- `post-merge-sweep.py:29` opens `python3 -I - <<'PYEOF'` and `:293` closes it — the whole Python
  ...
  re-derived at source, `post-merge-sweep.py:214-232`.) Nothing there is importable; no
...
**(b) The citation was corrected inside the same amend**, per instruction — `post-merge-sweep.py:29-43`
...
- **T-11** scopes itself in its first line to "the unit floor the test matrix requires for T-02,
  ...
  one T-07 reference (`post-merge-sweep.py:223` as a branch BE-10 also covers) claims no obligation.
...
of the shape**: `gh-close-gate.py:59` and `plan-sign-gate.py:43` both carry the comment *"THE
...
1. `D-12` stands **plan-local** and needs no sibling. Its footing is DEC-217's Over clause plus the
   structural fact that `post-merge-sweep.py`'s body admits no import at all — neither of which
...
2. The generalisable fix is **not an exemption but the conversion**: extract the two surviving
   heredoc bodies into `.py` files beside their shells, exactly as `gh-close-gate.py` and
