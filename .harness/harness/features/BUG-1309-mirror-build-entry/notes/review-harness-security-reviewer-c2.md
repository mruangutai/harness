...
5 insertions/1 deletion across those two files only). `merge-gate.py`, `merge-gate.py` and
...
merge-gate.py feature_schema.py` → empty; `git hash-object` on both files matches what cycle-1
...
**5. Gate's own failure modes → permit or refuse?**
  ...
  - **`merge-gate.py`'s own root-resolution failure is fail-CLOSED, not fail-open**: if
...
| `merge-gate.py` root-resolution failure | Denial of service | **true** (fails closed, blocks everything — availability concern, not a bypass) |
...
- `UnicodeDecodeError`/`local_branch()`-crash gaps from cycle-1: unchanged, still non-escalating
  (P-02), not re-raised.
- `merge-gate.py` fail-closed-on-broken-interpreter: an availability tradeoff, not a security gap;
