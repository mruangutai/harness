...
| `bugfix` + `touches_runtime_code` → `unit`; T-02, T-04, T-06, T-07 add none | T-02 → BE-11..BE-20; T-04 → BE-21..BE-24; T-06 → BE-01..BE-10 (`feature_schema.py:226`, `:324` are T-06's own runtime change, and `check-state.py:2002,2011` is INV-37 reading exactly those two) | **T-07 IS UNOWNED.** T-11 lists `depends_on: [T-02, T-03, T-04, T-06]` (`plan.yaml:1510`) and traces no T-07 REQ. BE-10 asserts the era SET, never the sweep's branch. And the branch is unreachable from `tests/unit/`: it lives inside the `python3 -I - <<'PYEOF'` heredoc opened at `post-merge-sweep.py:29`, so there is nothing to import. Per `harness-qa-gate` step 5 this resolves `missing` → FAIL, not a soft skip. |
...
(`post-merge-sweep.py:230-231`), so the case can never green again on the retention branch having fired.
...
2. **Era set exempts the three refusals; retention keys on the RECORDED value.** Untouched. BE-10 pins
   exact, case-sensitive, non-prefix membership — the property both `post-merge-sweep.py:223` and
