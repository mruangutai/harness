...
| SC-06 sweep retention keys on the recorded value | automated / integration | `test-post-merge-sweep.py` exit 0, 61 PASS, 0 FAIL. Both named cases present **verbatim**: `T-07 era-exempt recovery-required keeps the worktree` and `T-07 era-exempt absent build_entry is swept`. Retention keys on the value, never era: `post-merge-sweep.py:222-232` tests `entry is None and feature_id in ERA_EXEMPT` first, then `elif entry not in {opened, not-applicable, recovered-terminal}` — so era + `recovery-required` falls to the retain arm. Announcement text exists at `:224` (predates) and `:229-232` (names `github.build_entry=<value>`) | **met** (see advisory A2) |
