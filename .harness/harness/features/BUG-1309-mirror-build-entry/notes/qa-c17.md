...
test file's `fixture()`/`gate()` shape, pointed at each `merge-gate.py` in turn:
...
`github.repo: "acme/widgets"`. Drove `merge-gate.py` (the pinned copy) directly, exact commands
and output:
