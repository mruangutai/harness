...
- 2026-09-09: BUG-1309 ship phase. A `verify: uat` criterion arrived closed as an overall operator "pass" plus a judgement of the one sentence they cared about, with no per-step readout of the script's own five-step verdict rule. Recording it as MET was right (the human's judgement IS the method) but the honest record needed the fidelity limit named in the same paragraph and carried up as an OPTIONAL confirmation line, not as a blocker and not as a re-grade of the operator. Framing it as one strikeable line in the briefing cost nothing and kept the record true.
- 2026-09-09: BUG-1309. Both state-file shape gates refused my first write in this one phase: STATE.md at 121 lines against a 120 cap, handoff-ship.md at 61 against 60. Both times the overflow was inherited content (a 14-entry open-question list, a 5-bullet dead-ends list), not new prose. Measure the carried-forward list first and budget the new lines against it, rather than writing and re-trimming.
- 2026-09-09: BUG-1309. A previous briefing's backlog table that the operator never disposed of does NOT expire — its rows were still the only record of 13 residuals. Subsuming it into one freshly-numbered table with provenance beat appending a second table, and re-measuring two of its rows at the pin found one already fixed incidentally (a module-scope import moved into the function) and one still live, which is a cheap way to keep a stale row from wasting an operator's attention.
