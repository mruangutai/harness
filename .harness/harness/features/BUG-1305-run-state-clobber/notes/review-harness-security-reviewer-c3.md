...
Scope: `1b11bc18..5ed929bd`, `.claude/skills/harness/bin/check-domain.py` (18 ins / 4 del) +
...
Traced, not assumed:
- Domain authorization (`domain_check()`, `check-domain.py:819`) grants by **path glob only**
  (`team-config.yaml` `domain:` lists, walked at `check-domain.py:250-270` for the
...
- Content shape enforcement (`shape_problems`, `check-domain.py:1262`, DEC-150) is explicitly
  tool-agnostic by design — the RE_RUN_DIGEST comment at `check-domain.py:1297` states it
...
- The one tool-specific branch in the file (`_tool == "Write"`, `check-domain.py:630`, the
...
Both new messages (`check-domain.py:2059-2071`) are static strings: no file content, no
...
members of `SHAPE_PATTERNS` at `check-domain.py:1244`) via the **Edit** tool at PRE falls
through the *same* `elif _tool != "Write" or not target: sys.exit(0)` (`check-domain.py:2074`)
that existed before this diff — unconditional exit 0, no reconstruction, no shape check.
`plan.yaml` is separately covered by `approval_guard()` (`check-domain.py:559`, called from
...
rules from what landed, `check-domain.py:1104-1114` comment block, "too late to refuse").
...
- **F-1** (info, `outside-delta`): Edit on `feature.json`/`STATE.md`/`CLAUDE.md` still exits 0
  unconditionally at PRE with no content reconstruction or shape check (`check-domain.py:2074`),
