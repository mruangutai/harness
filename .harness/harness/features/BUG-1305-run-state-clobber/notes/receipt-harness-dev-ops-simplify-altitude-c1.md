...
| Allowed top-level checkpoint keys | `ALLOWED` (`check-domain.py:1522`) and `CHECKPOINT_KEYS` (`check-state.sh:1391`), each hand-listing `run_uid` | 2 | leave — pre-existing duplication (predates this diff), explicitly documented as an intentional vocabulary-only sync (D-02 comment at `check-domain.py:1515-1521`), and D-12 signed adding `run_uid` to both. Consolidating the two sets reaches outside this diff's scope |
