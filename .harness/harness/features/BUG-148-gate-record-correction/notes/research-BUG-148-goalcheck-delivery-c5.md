...
| SC-04 | automated (integration) | **met** | `env -u HARNESS_AGENT_TYPE python3 .agents/skills/harness/bin/run-unit-tests.py --kind integration`, `rc=$?` | **runner exit status = 0**; `^FAIL ` line count = **0**; `ok - test_committed_index_matches_a_fresh_regeneration` and `ok - test_no_amendment_construct_survives_in_the_authority` both present (output L884, L888). 69.7 s wall |
