...
- 2026-09-06: the dispatch enumerated two readers; check-state.py INV-32 (:534) demands should-not-exist + scope + goalcheck at approval, and BUG-148's goalcheck HAD run outside the validator run (notes/research-BUG-148-goalcheck-plan-c0.md, its F-2/F-4 cited by the panel). Recorded it as a third reader entry rather than shipping a record that reddens INV-32 at signature.
- 2026-09-06: reader entries key on `reader:`, never `id:` — check-state.py builds by_reader off `reader` (:539), so an `id:`-keyed entry is invisible to the gate and reads as a reader that never ran.
...
- 2026-09-07 (BUG-148 goal-check): two bash calls issued in one parallel block — the first writing /tmp/feat05_state.txt, the second grepping it — raced: every grep returned empty and exit 1, which reads exactly like 'the required strings are absent'. Sequence a producing command and its consumer in ONE call when grading by grep.
