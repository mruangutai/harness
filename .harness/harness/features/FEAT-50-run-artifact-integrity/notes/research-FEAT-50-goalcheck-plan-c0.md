...
| 4 | Three canonical commands exit 0 | `partly delivered` — **handling is `delivered`** | The two suite commands are pinned by SC-10. `check-state.py` is not claimed: SC-11 grades "no violation row names FEAT-50" (form (c)) and says so; SC-12 grades that a ruling or a recorded deferral exists; the three options, PRINCIPLES rule 15's veto on (b), and "neither pm nor the orchestrator may choose" are in `BRIEF.md:226-251`; the gap is restated in `## Verification gaps`. Re-measured: `check-state.py` exits 1, 51 `INV-` lines, **zero** `FEAT-50` mentions today. The decider is named, the disclosure is at the signature, no option is taken silently. Residual: SC-11's teeth arrive only after approval, and the `panel:` key it then requires is owed by no `T-NN` (**F-09**) |
...
`_run_domain and not _no_parser` (`check-domain.py:872`) (**F-04**), and "otherwise ALLOWED" does not
say whether `outcome == "shared"` — a second allowance at `check-domain.py:843` — counts (**F-06**).
Verified as **not** dead: T-04's PRE route does reach `shape_problems()` unconditionally
(`check-domain.py:1361-1370`, no `has_shape_rules` gate, unlike POST at `:1377`) and problems become
...
- **F-01 · med · governance.** pm wrote inside `plan.yaml`'s `approval:` mapping: `rulings: []` plus
  ...
  `check-domain.py` denies an agent's write of this mapping"; `harness-spec-driven` says "Never the
...
- **F-03 · med · dead remedy.** T-03 says "inside the `if _run_domain:` block, positioned so it runs
  ...
  else". That block is the fail-closed **import** guard at `check-domain.py:366-400`; `_verdict`
  does not exist there. The only site that can host the check is the allow branch inside
  `domain_check()` at `check-domain.py:833-841`, which already hosts a narrowing fragment denial
...
- **F-04 · low · undisclosed scope.** `domain_check()` is called under
  `if _run_domain and not _no_parser:` (`check-domain.py:872`), so in a PyYAML-bootstrap session
  T-03's binding does not run at all while T-04's digest rule, which lives in the shape phase, still
  does. Consequence: a documented escape hatch (`check-domain.py:731-739`) silently disables one of
...
- **F-06 · low · incomplete rule.** T-03 branches on the verdict being "otherwise ALLOWED".
  `check-domain.py` has two allowances: `outcome == "allow"` (`:835`) and `outcome == "shared"`
...
- **F-11 · low · incomplete anchor.** T-04 calls the tuple threading "the single call site's
  `targets` tuples (check-domain.py:1359-1420, 1546-1547)". There are **three** construction sites:
