...
The tool-route checkout binding (`check-domain.py`) is sound for every shape tested. But its
Bash-route twin (`bash-write-guard.py`, T-09/REQ-08) omits the `feature_checkout_guard` call on
...
|---|---|---|
| Plain absolute path, main checkout, domain `allow` | **DENY (2)**, correct | `check-domain.py:877` `feature_checkout_guard` called on `allow` |
| Same, domain outcome `shared` (Write tool) | **DENY (2)**, correct — verified live | `check-domain.py:881` |
| Same, domain outcome `shared` (**Bash** tool, `echo hi >`) | **ALLOW (0) — BUG** | `bash-write-guard.py:780-787`: guard runs only for `("allow","not_a_domain_question")`, never for `"shared"` |
...
| Malformed JSON hook payload | both (pre-existing, unrelated to this diff) | `d = {}` → `target=""` → exits 0 before any gate is reached | `check-domain.py:161`, `bash-write-guard.py:79` |
| Digest-clobber prior-content read hits `OSError` (permission error, broken symlink, EISDIR) | check-domain.py digest guard (NEW, REQ-04) | Treated as `prior=""` → guard condition `if prior.strip()...` is false → **write allowed, clobber proceeds** — reproduces the exact #1058 loss under an unusual I/O precondition rather than "file absent" | `check-domain.py:1146-1147` |
| Digest-clobber prior-content read: file size | same | Fully read into memory with no bound; self-inflicted DoS at most (attacker needs prior write access to have inflated it) | `check-domain.py:1144` `.read()` |
| Digest-clobber deny message | same | Never echoes prior or new digest text to stderr/logs — no cross-agent content leakage | `check-domain.py:1148-1150`, verified by reading |
...
| Non-UTF-8 file content | digest guard | `errors="replace"` — no crash, degraded comparison only | `check-domain.py:1144` |

## Ranked findings

1. **[HIGH, must_fix]** `bash-write-guard.py` omits the checkout-binding check on the `shared`
   ...
   `feature_checkout_guard(rel, ap)` in the `shared` branch too, mirroring `check-domain.py:881`.
   `bash-write-guard.py:780-787`.
2. **[MED]** Digest-clobber guard (REQ-04) treats an `OSError` while reading the prior
   ...
   the genuinely-new-file case. `check-domain.py:1146-1147`.
...
4. **[INFO]** Unbounded read of the prior digest file on every `Write` to a `runs/*/digest.md`
   path; self-inflicted blast radius only. `check-domain.py:1144`.
...
- REQ-04 is `Write`-route-only (Edit/NotebookEdit/`cat >` unenforced) — disclosed in
  ...
  `Write` branch (`check-domain.py:1418-1426`).
...
VERDICT: FAIL
DIGEST:
  headline: "bash-write-guard.py's T-09 checkout binding skips the `shared` domain outcome — a feature-scoped shared path can be written straight into the main checkout via Bash while the Write-tool route correctly refuses it (verified live); REQ-08's route-completeness claim is false for that outcome."
  in_scope: true
  ...
  findings: 5
  must_fix:
    - "bash-write-guard.py:780-787 — feature_checkout_guard is never called on the `shared` outcome, unlike check-domain.py:881; a feature-scoped path classified `shared` bypasses the main-checkout refusal via Bash. Reproduced live."
  threat_model:
    - { boundary: "PreToolUse Write/Edit/NotebookEdit -> check-domain.py checkout binding", stride: T, mitigated: true }
    - { boundary: "PreToolUse Bash -> bash-write-guard.py checkout binding, allow/not_a_domain_question outcome", stride: T, mitigated: true }
    - { boundary: "PreToolUse Bash -> bash-write-guard.py checkout binding, shared outcome", stride: T, mitigated: false }
