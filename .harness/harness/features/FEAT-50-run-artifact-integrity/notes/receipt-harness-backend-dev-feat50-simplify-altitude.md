...
Python hook; #1057 lives in `check-domain.py`'s domain-verdict route (`domain_check()`, ALLOW/DENY
before content exists); #1058 lives in `check-domain.py`'s content-shape route (`shape_problems()`,
...
`check-domain.py` when the module that already owns "which checkout does this path stand in"
...
|---|---|---|---|---|
| ALT-01 | med | T-03 (`check-domain.py` intent, plan.yaml:279-322) | feature-id-from-path derivation + worktree-basename match is planned as inline logic in `check-domain.py`, not as a named function in `harness_boundary.py`, which the plan's own `files:` and `lanes:` never open | fold-in |
...
enumerates registered checkouts) — and `check-domain.py` already imports it (`harness_boundary`,
referenced at check-domain.py:740). T-03's intent (plan.yaml:288-291) instead has the gate itself
...
binds returns. `check-domain.py` is bash and unimportable, so the author reimplements the
...
`check-domain.py`'s existing `if _run_domain:` block. The regex `^\.harness/[^/]+/features/([^/]+)/`
and the `real()`-before-compare requirement move with it. `check-domain.py`'s side shrinks to: call
...
enforcement logic (it names `check-domain.py` and the observable behaviour — refused when content
...
`real`, `worktree_owner`, :102-593); `.claude/skills/harness/bin/check-domain.py`
