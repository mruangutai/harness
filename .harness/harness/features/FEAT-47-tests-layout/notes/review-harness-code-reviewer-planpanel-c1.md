...
- `.harness/harness/expertise/harness-code-reviewer.md:7` and
  ...
  `run-unit-tests.py`'s `UNIT_SCRIPTS`/`INTEGRATION_SCRIPTS` arrays "by exact name" as live guidance.
...
- Nothing in this plan has executed: `suite_layout.py`, both new test files, and the rewritten
  `run-unit-tests.py` don't exist yet. My soundness claims for T-05 (the pool-argument parse, the
