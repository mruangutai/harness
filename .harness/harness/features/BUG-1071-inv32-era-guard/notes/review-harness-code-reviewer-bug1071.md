...
(`check-state.py` +37, `test-check-state.py` +104/-2) — matches the Working Set precisely, no
scope creep.

- **Trust claims re-verified, not trusted.** Ran `python3 check-state.py` on the real tree:
...
- **Dead ends avoided, confirmed by diff + grep:** no `plan.yaml` anywhere is touched (diff
  ...
  check-state.py` → no matches — no grandfathering. Post-era plans missing a `panel:` block
  still `bad.append(...)` (unchanged code path at `check-state.py:218-220`, confirmed by
...
`check-state.py:199`. The literal `"2026-08-31"` is this repository's own bootstrap date
...
in its own `check-state.py` history, or a value stamped into `.harness/harness.json` at
...
`check-state.py:200-210`. Grepped the whole script and the loader for any other place
`approval.date` is required on an approved plan: **none exists.** The only other approval
check (`check-state.py:158-164`, INV-3/4/5) validates `approval.status` only.
...
`check-state.py:207-209`. The undated-approval note names the defect ("`approval.date` is
...
`check-state.py:199`. A literal string reassigned identically on every iteration of
...
through, unchanged, to the pre-existing panel-check code at `check-state.py:217+` — the guard
...
DIGEST:
  ...
  open_questions:
    - { id: Q1, question: "Should INV32_ERA_START be derived per-repo (git history of the INV-32 marker's introduction, or a .harness/harness.json field stamped at upgrade time) instead of a shared literal, given check-state.py is distributed to onboarded repositories with independent update cadences?", blocking: false }
