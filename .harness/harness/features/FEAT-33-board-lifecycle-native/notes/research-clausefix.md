...
(`check-domain.py`, `bash-write-guard.py`, `validate-digest.py`, `check-plan-routes.py`) — `check-state.py`
is not among them (ruling 4, 2026-08-23). And `plan.yaml` D-24 has T-22 editing `check-state.py` by
...
- `.claude/skills/harness/bin/check-state.py` -> exit 1, exactly ONE violation, the pre-existing and
