...
- G-02: WHEN assessing a test-matrix claim here DO read `run-unit-tests.sh`'s UNIT_SCRIPTS and INTEGRATION_SCRIPTS arrays, not `harness.json`'s detect globs — array membership decides which script actually executes; the globs only decide which kind the gate believes is required.
