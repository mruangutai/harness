...
## Patterns (max 15)
## Gotchas (max 15)
- G-01: WHEN gating a change to `team-config.yaml`'s per-agent repository-tier grants DO know the only regression protection is a one-shot task `verify:` block — `run-unit-tests.sh` never re-runs `check-domain.py --resolve` across all agents, so a dropped grant will not redden CI.
