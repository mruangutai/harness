...
opens with, verbs `add`, `replace` and `drop` (`cmd_ops`, `expertise-merge.py:484-533`). It shares
...
(`_validate_target_section`, `:170-178`).
...
section and only an `add` lengthens it (`_rebuild_section`, `:269-283`).
...
in any order yields the same file (`_resolve_all`, `:254-266`). Caps are checked **once, on the final
state** against the same `CAPS` §5.2 states (`_check_caps`, `:310-317`), which is what makes a drop
...
|---|---|
| The op's section + id names no entry in the base file | **10 MISSING TARGET** `section=<s> id=<id>` (`:203-212`) |
| The id appears twice in that section, or two ops in one proposal name the same target | **11 AMBIGUOUS TARGET** `section=<s> id=<id>` (`:213-251`) |
| The payload is not a JSON list, or an op omits a required key, carries a forbidden one, or names an unknown verb | **12 MALFORMED OPS** `op index=<i>: <reason>` (`:153-200`) |
