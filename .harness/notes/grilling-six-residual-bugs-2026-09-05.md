...
- Success for each issue is an approved BRIEF and plan, completed build and validation, merged pull request, ship closeout, closed source issue, passing issue-specific verification and PR checks, and `bash .claude/skills/harness/bin/check-state.sh` exiting 0.
