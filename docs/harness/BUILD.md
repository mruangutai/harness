...
| 14 | Router + **`.claude/agents/harness-orchestrator.md`** + `harness.json` + CLAUDE.md — **and size** | pending — DEC-120 made the orchestrator a *spawned agent*, so the sixteenth agent definition and the `/harness` entry doors are this task. The lifecycle doors are `/harness`, `/harness-plan` and `/harness-ship`; **`/harness-debug` is not one of them** — debugging is on-demand, invoked when there is a bug, not a stage of the build flow. CLAUDE.md is ~164k tok/feature, 3× the rule cost (DEC-105). **Also owns the question round-trip** (SPEC §2.1): the lead half is built — collation unions `open_questions` upward — but the half that remains is orchestrator behaviour: ask, write `.harness/notes/answers-<FEAT>-<runid>.md`, re-delegate via `resume_from`. No orchestrator, no round-trip. **Must also add `harness-orchestrator` to `validate-digest.py`'s `ALIAS` and `SCHEMAS`** — it is matched by the `harness-.*` `SubagentStop` matcher but has no schema, so it falls to the "no schema — passing through" branch and **the tier every digest routes through would be the one ungated agent** the day this task lands (found by the DEC-124 review panel) |
