...
| `PreToolUse` `exit 2` blocks a subagent write? | **YES** — 5/5 cases. Frontmatter declaration site still on the restart checklist |
...
### 0b — Domain-enforcement hook — VERIFIED, script shipped
...
**Result:** blocking works, and the stderr reason reaches the agent. But note what this does **not** buy:
the hook cannot see writes made via `Bash`, and all 9 doers hold it. **Serialization (SPEC §8.5) plus
`isolation: worktree` is the write-safety mechanism; the hook is a guardrail** (DEC-85). Do not read a
passing hook as proof of parallel safety.

The hook must also print the agent's **permitted globs** on rejection — a probe confirmed that naming
only the rejected path leaves an agent no basis for choosing a valid alternative (DEC-100b).

**One residual:** the tests used a `settings.json` hook, not an *agent-frontmatter* one, because agent
definitions are not live-reloaded (DEC-100a). The docs assert the frontmatter variant directly and the
blocking mechanism is proven, so only the declaration site is unconfirmed. **Confirm on the first
post-restart run.** Note also: a domain hook belongs in agent frontmatter, never `settings.json`, or it
governs the orchestrator too.
...
3. **One linear crew.** A minimal `crew/SKILL.md` runner + a single linear crew
   (`pm → backend-dev`) with `lead: eng-lead`, no gating or loop-back yet. Prove file-path state
   passing, lead→member dispatch, and the `lead:` host field.
