...
| 23 | **understand-codebase** — the codebase map (DEC-137) | pending — **blocks 17** (its first act). Build: `.harness/codebase/` layout + capped `INDEX.md`; extend `inject-expertise.sh` to inject the index at spawn; the multi-squad mapping playbook (role-authored views, documentor consolidates); the ship-refresh step (documentor marks stale + owning specialist rewrites its own sections, DEC-137 amendment); anchors + per-section dates throughout |
