...
| 10 | **Team runner + lead collation, PROVEN** | **in progress** — runner built: gating, `loop_back`, parallel dispatch, all from spawn records (DEC-116, DEC-117). Digest enforcement wired and proven (DEC-121/122/123): the `SubagentStop` hook blocks a malformed return, and the lead verdict roll-up is computed rather than trusted. **What is NOT proven, and is the whole of what remains: a lead actually dispatching its members, collecting their digests, assessing them, and emitting a valid team digest to `<run>/digest.md`.** Everything so far tests the *format* in isolation. Found while setting that run up: **no lead could reach the runner at all** — no `Skill` tool, `harness-team` absent from every lead's `skills:`, and no lead file even naming its path. Wired; needs a restart to verify, since agent definitions are not live-reloaded |
...
| 14 | Router + **`.claude/agents/harness-orchestrator.md`** + `harness.json` + CLAUDE.md — **and size** | pending — DEC-120 made the orchestrator a *spawned agent*, so the sixteenth agent definition and the `/harness` entry doors are this task. The lifecycle doors are `/harness`, `/harness-plan` and `/harness-ship`; **`/harness-debug` is not one of them** — debugging is on-demand, invoked when there is a bug, not a stage of the build flow. CLAUDE.md is ~164k tok/feature, 3× the rule cost (DEC-105). **Also owns the question round-trip** (SPEC §2.1): the lead half is built — collation unions `open_questions` upward — but the half that remains is orchestrator behaviour: ask, write `.harness/notes/answers-<FEAT>-<runid>.md`, re-delegate via `resume_from`. No orchestrator, no round-trip |
...
| 17 | Take the full workflow through its paces in kaya-ai | pending — **blocked on 10, which is itself blocked on 14** (3, 12, 13 done). The earlier "blocked only on 10" was wrong: 10's remainder needs an orchestrator that does not exist yet. Measure the DEC-114 open question: is the orchestrator really ~80% of spend? |
