...
| 10 | Team runner + four v1 teams | **in progress** — runner done incl. gating and parallel dispatch, all proven from spawn records (DEC-116, DEC-117). **Scope corrected (DEC-118): only 2 of the 4 are teams** — `plan-feature` and `ship-feature` are orchestrator playbooks sequencing per-squad runs, since a team is single-squad by construction. Built: `review` team. **Digest enforcement is wired and proven** — `validate-digest.py` runs as a `SubagentStop` hook, the fourth mandatory prerequisite; an agent told to return bare `done` was blocked and had to correct itself, and the lead verdict roll-up is now computed rather than trusted (DEC-121, DEC-122, DEC-123). **`debug` moved out to task 20** — it is a standalone team, not on the critical path. Remaining: the two playbooks and the question round-trip, **both blocked on 14** — DEC-118 makes the playbooks orchestrator-sequenced, and the round-trip's `member → lead → orchestrator → main session` chain has its lead half built and no orchestrator to receive it |
