...
| 24 | **GitHub Issues integration** (DEC-138) | pending, design settled — asymmetric truth (Issues are pm's input; outbound-only after approval), orchestrator syncs at its checkpoints gated by `github.sync` in harness.json, gh-absent = skip-and-report. Build: `bin/gh-sync.py`, playbook sync points, pm intake rule, harness.json key. **Before 17** so kaya's first feature mirrors from day one |
