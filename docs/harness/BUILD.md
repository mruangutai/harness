...
| 23 | **understand-codebase** — the codebase map (DEC-137) | **built, unproven** — manifests carve the map by author (hook-proven: right author 0, wrong author 2); `inject-expertise.sh` injects the capped INDEX at every spawn (canary-proven both ways, 80-line backstop); mission `map` + the owner-respecting ship-refresh in the playbook; door route; `codebase-INDEX.md` template. **Proving it IS task 17's first act** — the kaya-ai map run |
| 24 | **GitHub Issues integration** (DEC-138) | **built** — `bin/gh-sync.py` (open/close-task/ship; idempotent; repo always pinned; env failure = SKIP exit 0, caller error = exit 1) + 17-case offline suite against a fake gh asserting exact outward calls; playbook sync points; pm intake rule in harness-brief; door shipped-row; harness.json `github` block in template + init interview step + INV-13 (sync-on-without-repo is a violation, missing block nags for --upgrade). **Remaining: one live smoke vs a scratch repo** (fake-gh can't prove real API shapes), then the full loop on kaya-ai's first feature |
