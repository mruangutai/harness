...
**Crew overrides live in `.harness/crews/`, not `.claude/skills/harness/crews/`.** The skill tree is
this repository's own source, rewritten by harness development itself — so an override placed inside
it is whatever the next harness change leaves behind, and nobody editing harness code owes it a
thought. The override directory is project-owned state that harness development never edits. The
precedence rule BUILD asked for ("project-local overrides global") only holds if the override sits
there. Recorded in both manifests as `paths.crew_overrides`; the runner (task 10) resolves it first.
