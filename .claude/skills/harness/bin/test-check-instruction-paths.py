...
def case_scope_and_debug_read():
    ...
    debug_line = next(line for line in open(backend, encoding="utf-8") if "harness-systematic-debugging/SKILL.md" in line).strip()
    debug_path = debug_line.split("`", 2)[1].replace("<HARNESS_CONTROL_PLANE_ROOT>", REPO_ROOT)
    check("product clone can read anchored systematic-debugging skill", os.path.isfile(debug_path) and "harness-systematic-debugging" in open(debug_path, encoding="utf-8").read(200), debug_path)
