...
1. **Build entry.** Require the signature-created `feature.json` `github.build_entry` receipt.
   ...
   still gates merge until that recovery succeeds. Then run
   `gh-sync.py status <feature-dir> building` for the complete card set before any task dispatch.
...
6. **The fix loop.** On `must_fix`, pin nothing. Run
   ...
   **naming the owning dev** — the `execution_agent` of the task each must-fix finding cites, read
   ...
   `review_sha`. At the next validation boundary, run
   `gh-sync.py status <feature-dir> review` before any subsequent validation dispatch or result handling.
   Each round is one `cycles_used` and one `regate` judgement. **The loop runs inside the
   operator's rework ruling** — `feature.json` `rework` `rounds` and `wall_clock_minutes`
