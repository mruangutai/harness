...
**The sanctioned exception:** the harness observations log is one file every worker appends to, because a single ordered record *is* the invariant. `.claude/skills/harness/bin/observations-merge.py` takes an exclusive lock, computes an order-preserving union, and writes it back atomically; workers go through it and never write the log directly. That is step 3 done structurally, after step 2 was honestly answered "yes, one object".
