#!/usr/bin/env python3
"""Guards two invariants of FEAT-37 (lead-stop-and-wake) that survive DEC-233:
...
The former "playbook" group pinned harness-team/SKILL.md's end-turn/wake conduct, which
was the Claude Code compatibility host's lifecycle. Under OMP a lead never ends its turn
with a child live — the task call blocks (DEC-204) — so DEC-233 retired that group with the
host; test-orchestrator-playbook.py and the harness-team prose carry the blocking rule.
...
# ---------------------------------------------------------------------------
# --self-check — proves the --only guard cannot grade an empty match as ALL PASS
# ---------------------------------------------------------------------------
...
USAGE = "usage: test-lead-stop-and-wake.py [--self-check | --group {bound,coverage} [--only PATH]]"
...
def main(argv):
    ...
    if group is not None and group not in ("bound", "coverage"):
        print(USAGE, file=sys.stderr)
        return 2
